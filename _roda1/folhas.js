...
function f0(d){
  ...
  c.innerHTML =
    ...
    '<div class="cena"><i class="aro"></i><div class="miolo">L</div><div class="cporbe">' + '<div class="it" style="left:50.0%;top:8.0%">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ro_lata.png?v=' + V + '" alt="">' + '<span class="rt">LA</span>' + '</div>' + '<div class="it" style="left:89.9%;top:37.0%">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ro_leao.png?v=' + V + '" alt="">' + '<span class="rt">LE</span>' + '</div>' + '<div class="it" style="left:74.7%;top:84.0%">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ro_limao.png?v=' + V + '" alt="">' + '<span class="rt">LI</span>' + '</div>' + '<div class="it" style="left:25.3%;top:84.0%">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ro_lobo.png?v=' + V + '" alt="">' + '<span class="rt">LO</span>' + '</div>' + '<div class="it" style="left:10.1%;top:37.0%">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ro_lupa.png?v=' + V + '" alt="">' + '<span class="rt">LU</span>' + '</div>' + '</div></div>' +
...
function rolaParaCruz(){
  ...
  /* ⚠️⚠️ LÊ AS DUAS PELO `window`, e isto NÃO é preciosismo: escrito como
     ...
     que quatro publicações minhas falharam seguidas hoje. Como `CRUZ` e `ATIVA`
     são `var` globais, elas são propriedades de `window`, e ler por ali
     funciona igual e é declarado. */
  var cs = [], i, andando = 0;
...
function ativaLetras(vagas, certa, id, fc, fd){
  ...
  _dicaTec("Escreva as duas letras do começo");
  falar("escreva");
}
...
/* ---------- teclado de letras (folha 8) ---------- */function ativa(q, certa, id, fc, fd){
  ...
  setTimeout(function(){
    ...
    /* ⚠️⚠️ ERA `E.cels` — a fila de casinhas da CRUZADINHA — dentro da função
       do teclado de SÍLABA, que escreve numa quadra só (`ATIVA.q`). O `E` não
       existe aqui: o ESLint acusava `'E' is not defined` e, com ele vermelho, o
       portão 0a2 dentro do `entregar.yml` segurava a publicação do repositório
       INTEIRO — quatro corridas minhas falharam por isto hoje. Na tela não dava
       erro visível porque o `setTimeout` engolia a exceção. */
    var _a = window.ATIVA;
    var cs = ((_a && _a.vagas) || (_a && _a.q ? [_a.q] : []))
             .filter(function(c){ return c && c.getBoundingClientRect; });
    if(!cs.length) return;
    /* ⌨️ SEM TECLADO DESENHADO (17/set/2026) nao ha retangulo para descontar.
       No celular o teclado DO APARELHO cobre a metade de baixo do mesmo jeito,
       entao a casinha vai para o MEIO — serve nos dois casos e nao mede nada. */
    try{ cs[0].scrollIntoView({block: "center", behavior: "smooth"}); }
    catch(e){ try{ cs[0].scrollIntoView(); }catch(e2){} }
  }, 60);
