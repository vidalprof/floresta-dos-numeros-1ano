...
function rolaParaCruz(){
  /* ⚠️ VAZIA DE PROPÓSITO: quem rola agora é o navegador, ao focar o
     campo do teclado do aparelho. Apagá-la quebraria as chamadas. */
}
function ativa(q, certa, id, fc, fd){
  /* ⚠️ SEM BARRA FIXA, SEM ROLAGEM FORÇADA (15/set/2026). O teclado da casa era
     fixo no pé da tela e tapava a palavra — daí o `comtec` e o `rolaParaCruz`.
     Agora quem abre é o teclado do APARELHO, e o navegador já rola a página
     sozinho para o campo com foco. As duas peças saíram juntas. */
  if(ATIVA) fechaAtiva();
  ATIVA = {q: q, val: "", certa: certa, id: id, fc: fc, fd: fd};
  q.className = "sq vaga ativa";
  q.innerHTML = '<span class="v"></span><span class="cursor"></span>';
  var c = poeCampoSobre(q);
  c.value = "";
  c.setAttribute("maxlength", "12");
  c.setAttribute("aria-label", "Escreva o nome do reino");
...
function confereLetras(){
  ...
  var A = ATIVA;
  if(mesmaPalavra(A.val, A.certa, A.exigeAcento)){
...
function campoTeclado(){
  ...
  TECIN.addEventListener("input", function(){
    if(!ATIVA) return;
    var v = (TECIN.value || "").toUpperCase().slice(0, 12);
    ATIVA.val = v; TECIN.value = v;
    var e = ATIVA.q.querySelector(".v"); if(e) e.textContent = v;
    if(v.length >= ATIVA.certa.length) setTimeout(confereLetras, 380);
  });
  TECIN.addEventListener("keydown", function(ev){
    if(ev.key === "Enter"){ ev.preventDefault(); confereLetras(); }
    else if(ev.key === "Escape"){ fechaAtiva(); }
  });
  TECIN.addEventListener("blur", function(){
    setTimeout(function(){ if(ATIVA && document.activeElement !== TECIN) fechaAtiva(); }, 120);
  });
...
function f0(d){
  ...
  c.innerHTML =
    ...
    '<div class="cena">' + '<div class="it" style="animation-delay:0.00s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/rn_reino_animal.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.35s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/rn_reino_plantae.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.70s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/rn_reino_fungi.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:1.05s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/rn_reino_protista.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:1.40s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/rn_reino_monera.png?v=' + V + '" alt="">' + '' + '</div>' + '</div><div class="coroa"></div>' +
