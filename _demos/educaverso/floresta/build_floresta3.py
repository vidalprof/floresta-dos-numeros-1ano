...
var SRC=__SRC_JSON__, FALAS=__FALAS_JSON__;
...
function frame(ts){if(ult===null)ult=ts;var dt=Math.min(.05,(ts-ult)/1000);ult=ts;var t=ts;
 ...
 cx.restore();
 // ---- clima/luz (tela) ----
 var lg=cx.createLinearGradient(0,0,0,VH);lg.addColorStop(0,"rgba(255,236,170,.10)");lg.addColorStop(1,"rgba(30,20,60,.12)");cx.fillStyle=lg;cx.fillRect(0,0,VW,VH);
...
HTML=HTML.replace("__SRC_JSON__",SRC_JSON).replace("__FALAS_JSON__",FALAS_JSON)
out=os.path.join(S,"byte-floresta3.html")
open(out,"w",encoding="utf-8").write(HTML)
print("OK ->",out,"(",round(len(HTML)/1024),"KB ) imgs:",sorted(SRCJS.keys()),"falas:",sorted(FAL.keys()))
