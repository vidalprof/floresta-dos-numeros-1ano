...
function frame(ts){if(ult===null)ult=ts;var dt=Math.min(.05,(ts-ult)/1000);ult=ts;var t=ts;
 ...
 if(byte.mov){var L=Math.sqrt(vx*vx+vy*vy);vx/=L;vy/=L;var sp=118;
  byte.x=Math.max(DL,Math.min(DR,byte.x+vx*sp*dt));byte.y=Math.max(DT,Math.min(DB,byte.y+vy*sp*dt));
