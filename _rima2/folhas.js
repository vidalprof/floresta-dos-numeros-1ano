...
function f0(d){
  /* CAPA COM IDENTIDADE PRÓPRIA — gerada por _padrao/identidade_capa.py (editar lá).
     Cena: a corrente de rimas: as figuras da parlenda HOJE É DOMINGO ligadas por um elo, e um pulso que corre de uma para a outra (recortadas da folha de papel d31). O título entra letra a letra (pula), palavra por palavra
     (nowrap, para não quebrar no meio); as figuras são as do próprio caderno. */
  var c = el("div", "capa"), nome = "A Parlenda que Rima", k, letras = "", pos = 0;
  ...
  c.innerHTML =
    ...
    '<div class="sub">Língua Portuguesa &middot; 2º ano &middot; 35 folhas sobre parlendas, quadrinhas e rimas</div>' +
    '<div class="cena">' + '<div class="it" style="animation-delay:0.00s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/qd_cachimbo.png?v=' + V + '" alt="">' + '' + '</div>' + '<i class="elo"></i>' + '<div class="it" style="animation-delay:0.35s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/qd_jarro.png?v=' + V + '" alt="">' + '' + '</div>' + '<i class="elo"></i>' + '<div class="it" style="animation-delay:0.70s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/qd_sino.png?v=' + V + '" alt="">' + '' + '</div>' + '<i class="elo"></i>' + '<div class="it" style="animation-delay:1.05s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/qd_touro.png?v=' + V + '" alt="">' + '' + '</div>' + '</div>' +
...
function fim(){
  ...
  for(ke = 0; ke < 3; ke++)
    est += '<img src="img/qd_selo' + (ke < cheias ? "" : "_off") + '.png?v=' + VIMG + '" alt="" draggable="false">';
...
var OBJETIVOS = [
  ...
  {n: "Reconhecer que duas palavras terminam com o mesmo som", f: [1, 2, 3],
   ok: "reconhece, de ouvido e pela escrita, que duas palavras terminam igual"},
  {n: "Completar o verso com a palavra que rima", f: [4, 5, 6],
   ok: "completa o verso da parlenda com a palavra que rima"},
  {n: "Achar as palavras que rimam dentro de uma parlenda", f: [7, 8, 9],
   ok: "acha no texto versificado as palavras que rimam entre si"},
  {n: "Reconhecer o verso como a linha da parlenda", f: [10, 11, 12],
   ok: "põe os versos na ordem e junta o começo ao fim do verso"},
  {n: "Escrever a palavra que rima e fecha o verso", f: [13, 14, 15],
   ok: "escreve a palavra que fecha o verso rimando"},
  {n: "Produzir uma rima sem lista de opções", f: [16, 17],
   ok: "produz uma rima sua, sem escolher de uma lista"},
  {n: "Distinguir o que rima do que não rima", f: [18, 28, 29, 30],
   ok: "separa os pares que rimam dos que só se parecem"},
  {n: "Ler a figura no lugar da palavra dentro do verso", f: [19, 20, 21],
   ok: "lê a figura no lugar da palavra e escreve o nome dela no verso"},
  {n: "Compreender o que a parlenda conta", f: [22, 23, 24],
   ok: "responde sobre quem, o quê e por quê dentro do texto poético"},
  {n: "Achar as palavras que rimam nos jogos de letras", f: [25, 26, 27],
   ok: "acha as rimas na grade de letras e na cruzadinha"},
  {n: "Trocar a rima mantendo o verso rimado", f: [31, 32, 33],
   ok: "troca a palavra do fim por outra que também rima"},
  {n: "Escrever o próprio verso e nomear rima, verso e estrofe", f: [34, 35],
   ok: "escreve o próprio verso rimado e usa os nomes rima, verso e estrofe"}
];
...
function figOu(f, cls){ return f ? img("qd_" + f + ".png", cls || "fig", "") : ""; }
...
/* ⚠️ A CHAVE DA OPÇÃO TRANSLITERA O ACENTO, NÃO O APAGA (lição paga no caderno
   do grandão e do pequenininho, 18/set/2026): o `chaveQuadro` da casa tira tudo
   que não é a-z, e aqui isso juntaria `pó` e `pé`, `mão` e `mao`, `vó` e `vo`.
   Numa folha de RIMA, que vive de fim de palavra, isso faria a opção errada
   valer como certa. O `ch()` do gerar_falas faz a mesma conta. */
function chavePal(w){
...
/* ---------- 1, 2 e 18 — QUAL RIMA (com figura, sem figura, e o intruso) ---- */
function montaRima(d, pi, DR, pede, comFig){
  ST.folha["p" + pi].forEach(function(k, i){
    var X = DR[k], id = "n" + pi + "_" + i, box = item(i + 1);
...
function f1(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Diga o nome da figura em voz alta. Depois toque na palavra que <b>termina com o mesmo pedacinho</b>.", "p" + pi + "enun");
  montaRima(d, pi, RIMFIG, function(X){ return "Qual rima com <b>" + X.n + "</b>?"; }, true);
}
function f2(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora sem desenho: só o ouvido. Diga a palavra e cada opção em voz alta.", "p" + pi + "enun");
  montaRima(d, pi, RIMPAL, function(X){ return "Qual rima com <b>" + X.n + "</b>?"; }, false);
}
function f18(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Nestas três, <b>duas</b> terminam igual e <b>uma</b> não. Toque na que não combina.", "p" + pi + "enun");
  montaRima(d, pi, INTRUSO, function(){ return "Qual é a <b>intrusa</b>?"; }, false);
}
...
function f3(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Toque numa figura e depois na palavra que <b>rima</b> com ela.", "p" + pi + "enun");
  montaLigFig(d, pi, LIGR);
}
function f11(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Cada verso está partido ao meio. Ligue o <b>começo</b> ao <b>fim</b> que rima com ele.", "p" + pi + "enun");
  var pares = ST.folha["p" + pi][0].map(function(k){
    var L = LIGV[k];
...
function montaLacuna(d, pi, DF, falaPref, comFig){
  ST.folha["p" + pi].forEach(function(k, i){
    var X = DF[k], id = "n" + pi + "_" + i, box = item(i + 1);
    if(comFig && X.fig) box.appendChild(el("div", "cena1", figOu(X.fig, "figacao")));
    var lin = el("div", "enunlin");
    lin.appendChild(el("div", "frasegrande", X.f.replace("___", '<i class="lacuna peq"></i>')));
    lin.appendChild(botaoSom("Ouvir o verso", function(){ falar(falaPref + k); }));
    box.appendChild(lin);
    opcoes(box, pi, id, opsPal(X.ops), chavePal(X.r), "pal",
           "certo" + pi + "_" + k, "dica" + pi + "_" + k);
    fechaItem(d, box, id);
...
function f4(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "A parlenda perdeu a última palavra de cada verso. A figura ajuda: toque na palavra que <b>rima</b>.", "p" + pi + "enun");
  montaLacuna(d, pi, VERSO1, "fra_", true);
}
function f5(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora sem figura, em três parlendas diferentes. Diga o verso em voz alta antes de escolher.", "p" + pi + "enun");
  montaLacuna(d, pi, VERSO2, "fra_", false);
}
function f12(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Só uma das três fecha o verso <b>rimando</b>. As outras cabem no sentido, mas não no som.", "p" + pi + "enun");
  montaLacuna(d, pi, FECHA, "fra_", false);
}
function f19(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "No lugar da última palavra tem um <b>desenho</b>. Toque na palavra que ele quer dizer.", "p" + pi + "enun");
  montaLacuna(d, pi, ENIG, "fra_", true);
}
function f22(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Você já leu esta parlenda na folha 8. Agora responda: o que ela conta?", "p" + pi + "enun");
  montaLacuna(d, pi, PERG1, "fra_", false);
}
function f23(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Leia com atenção: <i>O macaco foi à feira, não teve o que comprar. Comprou uma cadeira pra comadre se sentar. A cadeira esborrachou, coitada da comadre, foi parar no corredor.</i>", "p" + pi + "enun");
  montaLacuna(d, pi, PERG2, "fra_", false);
}
function f24(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Leia a quadrinha: <i>Rato Luca rói o queijo, a vaca faz angu. O gato chama o pato pra festa do tatu.</i>", "p" + pi + "enun");
  montaLacuna(d, pi, PERG3, "fra_", false);
}
function f31(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "E se a gente <b>trocar</b> a palavra do fim? A parlenda tem que continuar rimando. Qual serve?", "p" + pi + "enun");
  montaLacuna(d, pi, TROCA, "fra_", false);
}

/* ---------- 6 e 32 — PUXE A PALAVRA PARA DENTRO DO VERSO ---------- */
function montaSolta(d, pi, DS){
  var lista = ST.folha["p" + pi];
  pegaSolta(d, pi,
    lista.map(function(k){ return {k: k, html: '<span class="cartex">' + DS[k].v + "</span>",
                                   fala: "vrs_" + k}; }),
    lista.map(function(k){ return {k: k, alvo: k, rot: DS[k].rot, aria: DS[k].rot}; }),
    "vs", function(I){ return "pal_" + chavePal(I.rot); }, "frase");
}
function f6(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Cada verso está sem o fim. <b>Puxe</b> a palavra até o verso dela, ou toque numa e depois no outro.", "p" + pi + "enun");
  montaSolta(d, pi, SOLTA1);
}
function f32(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora a palavra do fim é <b>nova</b>: não é a da parlenda de sempre. Mas ainda rima. Puxe cada uma para o seu verso.", "p" + pi + "enun");
  montaSolta(d, pi, SOLTA2);
}

/* ---------- 7, 8 e 9 — ACHAR A RIMA DENTRO DO TEXTO ---------- */
function f7(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Leia a parlenda e toque em " + TXT.tx1.pede + ". Depois confira.", "p" + pi + "enun");
  noTexto(d, pi, TXT.tx1, "certo" + pi + "_t", "dica" + pi + "_t");
}
function f8(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Nesta, toque só em " + TXT.tx2.pede + ". Depois confira.", "p" + pi + "enun");
  noTexto(d, pi, TXT.tx2, "certo" + pi + "_t", "dica" + pi + "_t");
}
function f9(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Esta tem <b>três pares</b> escondidos. Toque em " + TXT.tx3.pede + ". Depois confira.", "p" + pi + "enun");
  noTexto(d, pi, TXT.tx3, "certo" + pi + "_t", "dica" + pi + "_t");
}

/* ---------- 10 — PÔR OS VERSOS NA ORDEM ---------- */
function f10(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "A parlenda foi <b>fatiada</b>. Puxe cada verso para o lugar dele, do primeiro ao último.", "p" + pi + "enun");
  var lista = ST.folha["p" + pi];
  /* ⚠️⚠️ OS VERSOS TÊM DE VIR EMBARALHADOS, e isto é um defeito que a FOTO
     pegou: o `pegaSolta` embaralha os ALVOS, mas não os itens. Nesta folha os
     itens SÃO os versos, e eles saíam na ordem certa, de cima para baixo — a
     criança acertava lendo a lista, sem pensar na parlenda. É a única folha do
     caderno em que a ordem do banco entrega a resposta. */
  pegaSolta(d, pi,
    lista.map(function(k){ return {k: k, html: '<span class="cartex">' + ORDEM[k].pos + "</span>",
                                   fala: "pos_" + k}; }),
    baralha(lista.slice(0)).map(function(k){ return {k: k, alvo: k, rot: ORDEM[k].v, aria: ORDEM[k].v}; }),
    "ord", function(I){ return "vrs_" + I.k; }, "frase");
}
...
function f13(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora você escreve. As casinhas dizem quantas letras tem a palavra que <b>fecha o verso</b>.", "p" + pi + "enun");
  montaGrade(d, pi, GRD1, "Escreva a palavra que rima", false);
}
function f14(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Outras quatro parlendas, o mesmo trabalho: escreva a palavra do fim.", "p" + pi + "enun");
  montaGrade(d, pi, GRD2, "Escreva a palavra que rima", false);
}
function f15(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Olhe o desenho e <b>escreva</b> o nome dele. A pista diz com qual palavra ele rima.", "p" + pi + "enun");
  montaGrade(d, pi, GRD3, "Escreva o nome da figura", true);
}
function f20(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "O desenho está dentro do verso, no lugar da palavra. <b>Escreva</b> o nome dele.", "p" + pi + "enun");
  montaGrade(d, pi, GRD4, "Escreva o nome da figura", true);
}
function f21(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Sem desenho agora: só o verso e a pista. <b>Escreva</b> a palavra que falta.", "p" + pi + "enun");
  montaGrade(d, pi, GRD5, "Escreva a palavra que rima", false);
}
function f33(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora a palavra nova é sua para escrever. Ela tem que <b>rimar</b> igual à antiga.", "p" + pi + "enun");
  montaGrade(d, pi, GRD6, "Escreva a rima nova", false);
}

/* ---------- 16, 17 e 34 — A RIMA É SUA (vale qualquer uma da lista) ------- */
function montaLivre(d, pi, DL){
  ST.folha["p" + pi].forEach(function(k, i){
    var X = DL[k], id = "n" + pi + "_" + i, box = item(i + 1);
    var lin = el("div", "enunlin");
    lin.appendChild(el("div", "ajuda", "Escreva " + X.q + "."));
    lin.appendChild(botaoSom("Ouvir o pedido", function(){ falar("prd_" + k); }));
    box.appendChild(lin);
    gradeEscrever(box, id, pi, k, X.ok[0], "Escreva a sua rima", X.ok);
    fechaItem(d, box, id);
  });
}
function f16(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora não há lista: a rima é sua. Pense numa palavra que termine igual e escreva.", "p" + pi + "enun");
  montaLivre(d, pi, LIVRE1);
}
function f17(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Mais quatro. Vale qualquer palavra de verdade que termine com o mesmo pedacinho.", "p" + pi + "enun");
  montaLivre(d, pi, LIVRE2);
}
function f34(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Agora a quadrinha é sua. Escreva a palavra que <b>fecha</b> o seu verso, rimando.", "p" + pi + "enun");
  montaLivre(d, pi, PROD);
}

/* ---------- 25 e 26 — OS CAÇA-RIMAS ---------- */
function f25(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Ache na grade a palavra que a pista pede: toque na <b>primeira</b> letra e depois na <b>última</b>.", "p" + pi + "enun");
  cacaPalavras(d, pi, CACA, "Ache a que");
}
function f26(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Outra grade. Nela as palavras estão <b>sem acento</b>: procure CHAO, não chão.", "p" + pi + "enun");
  cacaPalavras(d, pi, CACA2, "Ache a que");
}

/* ---------- 27 — A CRUZADINHA ---------- */
function f27(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Toque numa pista, escute e escreva a palavra que rima.", "p" + pi + "enun");
  cruzadinha(d, pi, CRZD, "crz_");
}

/* ---------- 28 e 29 — AS GAVETAS ---------- */
function f28(d, pi){ gavetas(d, pi, "gA", "Leia o par em voz alta. As duas palavras terminam igual, ou não? Leve para a gaveta certa."); }
function f29(d, pi){ gavetas(d, pi, "gB", "Agora são <b>três</b> gavetas, uma para cada fim. Leia a palavra e escute o fim dela."); }

/* ---------- 30 — MARQUE VÁRIAS E CONFIRA ---------- */
function f30(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Em cada grupo há <b>três</b> que rimam e três que não. Marque as três e toque em Conferir.", "p" + pi + "enun");
  ST.folha["p" + pi].forEach(function(k, i){
...
function f35(d, pi){
  faixa(d, pi, NOMES[pi - 1]);
  enunciado(d, pi, "Você já fez tudo isto sem saber os nomes. Agora eles: leve cada exemplo para a linha dele.", "p" + pi + "enun");
