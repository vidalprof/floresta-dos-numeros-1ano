...
- **Imagens da professora → pasta `_imagens/`:** o Claude não gera imagem (só o prompt) e a prof às vezes não anexa no chat. Ela sobe os arquivos direto no repo em `_imagens/` (github.com, funciona no celular), nomes simples, e avisa. Claude puxa → Pillow (autocrop/otimiza) → embute base64. Sem imagem? Desenhar mascote/figuras em SVG (vetorial), zero upload. Guia em `_imagens/LEIA-ME.md`.
