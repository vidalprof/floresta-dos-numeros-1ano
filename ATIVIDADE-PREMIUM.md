...
- **Imagem de abertura/final como medalhão REDONDO:** foto grande da tela inicial/final fica melhor como círculo (`border-radius:50%`, quadrado ~220px→260px, `object-fit:cover`+`object-position:center` p/ recortar no centro sem distorcer), com borda no tom da atividade, sombra e leve flutuar (`@keyframes boia`). Prefixos `-o-`/`-webkit-`. Conferir por screenshot que o recorte pega o mascote.
