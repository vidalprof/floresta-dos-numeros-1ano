...
- **Botão de avançar VISÍVEL porém DESABILITADO até a fala terminar (lição paga — pedida várias vezes):** "o botão aparece, mas só habilita quando o áudio acaba, bem pausado, sem cortar nada". NÃO deixar o botão habilitado apostando que "clicar não corta" — a criança clica e corta. Renderizar o botão já visível com `disabled` + classe de espera (opacidade menor) e só habilitar depois do fim REAL da narração. Como o elogio/streak/nível/conquista entram na fila DEPOIS (por `setTimeout`), um poll que checa "fila vazia" uma vez pode liberar numa fresta de silêncio antes da comemoração começar — usar um helper que só libera após **silêncio CONTÍNUO ~650ms** (mínimo inicial ~900ms), zerando a contagem se algo novo for enfileirado, com trava de segurança ~15s. Aplicar em TODAS as telas com narração + botão: feedback de acerto, "Começar!", fim de fase (inclusive botões secundários), ofertas de Desafio Extra/Treino e Grande Final.

**GRAFIA DE TELA ≠ GRAFIA FALADA (lição paga — sílabas/siglas):** o TTS SOLETRA maiúsculas curtas — "GA" sai "gê-á", "BO" sai "bê-ô", "SA-PA-TO" letra a letra — e não lê o sinal "+" (ou lê "mais" grudado e robótico). Quando o que aparece na TELA precisa estar em maiúsculas (destacar sílabas, siglas) mas isso soa errado falado, separar os dois:
- **Campos `fala` e `falaExplica` no desafio** (opcionais): a narração do enunciado usa `dd.fala` quando existe (senão `dd.enun`); o feedback usa `dd.falaExplica` quando existe (senão `dd.explica`). Assim a criança VÊ "GA mais TO formam qual palavra?" e OUVE "ga, mais to, formam qual palavra?".
- Na versão falada: sílabas em **minúsculas** (o TTS lê o SOM, não as letras), separadas por **vírgula** (micro-pausa natural), "+" escrito por extenso "mais", palavra-alvo como palavra inteira minúscula ("gato", "casa").
- Implementação: `falarDepois(dd.fala?dd.fala:dd.enun)` no render do desafio; `falar(el1+" "+(dd.falaExplica?dd.falaExplica:dd.explica),liberar)` no `feedbackAcerto`; o fallback de tempo sem voz mede o comprimento do texto FALADO (`falaExplica`), não o de tela.
- A `fala` obedece à mesma regra do enunciado: NÃO revela a resposta depois do último "?".

**RETICÊNCIAS QUEBRAM A FALA (lição paga):** "..." vira ". . ." depois do `limparFala` (que põe espaço após cada ponto), e o TTS lê cada ponto como uma pausa separada, picotando a frase. NUNCA usar "..." em texto que será falado — usar vírgula para a pausa ("Fale devagar, separando os pedaços: ga, to."). Na TELA "..." pode ficar (visual), desde que o que vai ao `falar()` seja a string limpa sem reticências (ex.: mostrar "Pense de novo..." mas `falar("Quase! Pense de novo.")`).

**NÃO NARRAR SÍMBOLOS NEM LACUNAS (lição paga):** o TTS lê em voz alta caracteres que são só visuais — o sublinhado de lacuna ("Complete: gato ___") sai como "underline underline", e parênteses, colchetes, chaves, aspas «»""'', asterisco, cerquilha, til/circunflexo soltos, barras, `< > | = + @ • ~` também são lidos. O `limparFala()` DEVE, antes de falar: trocar sequências de `_` por espaço (a lacuna vira pausa), colapsar reticências, trocar travessão `—`/`–` por vírgula e REMOVER os símbolos avulsos (viram espaço); e colar `([!?])\s*\.`→`$1`. O que vai ao `falar()` fica só com letras, números e `. , ! ? :`.
...
- **O dígito "2" também tem gênero na fala** (lição paga — "duas baldes"/"duas vacas"): o TTS lê "2" como "dois". Só **1 e 2** flexionam (um/uma, dois/duas); de 3 em diante é invariável. Quando a fala tiver "2" diante de palavra FEMININA, gerar "duas" (por `numExt`/`pluralG`, ou regex no `normPron`: `\b2\s+vacas\b`→"duas vacas", `\b2\s+vezes\b`→"duas vezes"; "vezes" é feminino).
- **Enunciado (texto + fala) bate com a FIGURA mostrada**: se a tela mostra ovos, o enunciado/fala não pode dizer "maçãs". Cuidado quando o objeto do desafio é sorteado/cicla — o texto tem que acompanhar. Auditar fase a fase: figura × enunciado × fala × explicação nomeiam o MESMO objeto.
...
- **COMPANHEIRO NO MAPA — posicionar ao LADO da ilha, nunca por `bottom` (lição paga: personagem cobrindo o nome da parada).** O companheiro é imagem RETRATO alta (~163×220). Se for `position:absolute` ancorado por `bottom` (com `.paradaNome` fluindo logo abaixo), o personagem desce sobre o título e o COBRE. Correto: ancorar por `top` na altura do MEIO do cenário (ex.: cenário 80px de altura → companheiro ~48px em `top:24px`, `right:-14px` para encostar na borda), `overflow:hidden`, e dar folga ao título (`.paradaNome{margin-top:4px}`). Auditar renderizando com um companheiro retrato ALTO (pior caso) a 360/320px: a base do companheiro tem que ficar ACIMA do topo de `.paradaNome` em TODAS as paradas. (Antes: `bottom:14px` → sobreposição em todas; depois: `top:24px` → ~18px de folga.)
...
19b. **Fala vs. tela conferida**: rodar CADA enunciado e explicação pelo `limparFala` e LER o resultado — nenhuma sigla/maiúscula curta sendo soletrada, nenhum "..." picotando, nenhum "+" cru. Onde a grafia falada difere da de tela, usar `fala`/`falaExplica` e reauditar que a `fala` não revela a resposta.
