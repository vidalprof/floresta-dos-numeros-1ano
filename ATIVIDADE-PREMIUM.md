...
4. **TRILHA** serpenteante:
   - Cada parada = cenário próprio (imagem) + companheiro (imagem de animal/personagem) + nome da fase + estado.
     ...
     **A ILHA É OBJETO FLUTUANTE, NÃO CENA RETANGULAR (lição paga):** a ilha é um OBJETO/conjunto temático RECORTADO (transparente, flutuando), igual aos outros assets — **sem fundo, sem retângulo/moldura** (a professora repetiu isso 3×). Se pedir "cena"/"cenário" ao ChatGPT, ele devolve um RETÂNGULO com fundo (parede, chão, céu, cortina, palco) e às vezes TEXTO ("CIRCUS") — inútil pra recorte. Prompt certo é RÍGIDO: *"só o objeto/conjunto do circo FLUTUANDO no fundo branco puro #FFFFFF; NÃO desenhe parede/chão/céu/cortina/palco/retângulo/fundo colorido; SEM sombras; SEM nenhuma palavra/letra/número (nem em cartazes)"*. Dá pra agrupar várias ilhas numa CARTELA (ex.: 2 cartelas de 6+5 numa grade bem espaçada) pra poupar a professora — depois recorto por célula.
     **A ILHA APARECE TAMBÉM NO NÓ DA TRILHA, não só na intro (lição paga):** o mapa mostra CADA parada como a sua ILHA (cenário) COM o companheiro ao lado — é isto que "cenário próprio + companheiro" quer dizer. Erro cometido: pôr a ilha só na abertura da fase e deixar o nó do mapa só com o bichinho no anel. Correto: no nó, a ilha flutua (com holofote suave) e o companheiro fica AO LADO (canto inferior); travada = cinza+cadeado, concluída = check, atual = glow + Teo caminhando. A ilha reaparece na intro da fase (§3).
     **HOLOFOTE NO CÓDIGO destaca a ilha no fundo escuro:** a ilha flutua sobre um brilho REDONDO suave (`radial-gradient` dourado/quente feito no CSS, NÃO na imagem) — parece a luz do picadeiro, tira a ilha do "solto sem graça" no fundo escuro **sem virar retângulo**, mais `drop-shadow` leve e flutuação (`@keyframes boia`, par `-webkit-`). Recorte: flood-fill do branco **a partir das BORDAS** (preserva brancos internos, ex.: listras da tenda/arco), depois autocrop — nunca threshold branco global (abre buracos nas listras claras).
...
**⚠️ NADA DE QUADRADO/CAIXA em volta dos itens da sombra (lição paga):** os dois lados ficam SOLTOS (sem borda, sem card, sem retângulo tracejado). Erro cometido: pôr um box tracejado na silhueta "pra marcar a zona de solta" — a professora rejeita quadrado em volta (regra geral dela). A zona de solta é o próprio `<div>` da silhueta (tamanho fixo + `data-drop`), INVISÍVEL, com a `img` em `pointer-events:none` — funciona sem desenhar caixa.
**SILHUETA PRETA EM FUNDO ESCURO PRECISA DE CONTORNO DE LUZ (lição paga):** silhueta preta sobre o fundo escuro da atividade SOME. Como não há caixa clara atrás, dar à `img` da silhueta um **contorno de luz suave** (`drop-shadow` claro duplo, ex.: `drop-shadow(0 0 3px rgba(255,240,205,.85)) drop-shadow(0 0 1px ...)`, par `-webkit-`) — a forma fica nítida (dá pra ver tanto a sombra quanto o bicho) SEM virar quadrado. Ao revelar, trocar por `drop-shadow` verde.

**FASE LIGAR OS PONTOS:** a criança TOCA nos números em ordem (só toque, NUNCA hover/passar o mouse — quebra no celular); ao ligar o último, some com as bolinhas e aparece a imagem PNG do objeto (animação de revelar por opacidade). O CONTORNO formado pelos pontos tem de ser a MESMA silhueta da imagem revelada — coordenadas extraídas da forma real da imagem (não chutadas), na mesma orientação; preferir figuras simples (estrela, concha). Proteger contra toque-duplo (o contador não pode pular). Fala com artigo de gênero por figura ("desenho da concha"). **Figuras GEOMÉTRICAS (estrela, balão/oval) podem ter os 10 pontos CALCULADOS por trigonometria** (a estrela = 5 pontas: raios externo/interno alternados a cada 36°; a bola/balão = pontos numa elipse) — é equivalente a "extrair da forma real" e casa com a imagem revelada. **Para figuras NÃO-geométricas (cartola, gravata…):** extrair o contorno da máscara alpha por **Moore-neighbor tracing** e reamostrar em 10 pontos por comprimento de arco (`cortar_ligarpontos.py`); mapear os pontos para o quadrado do jogo assumindo a figura CONTIDA (centralizada) num fração `fill` (ex.: 0.72) — e exibir a `<img>` com `max-width=max-height=fill%` pra casar. **PADRÃO: rodadas normais podem revelar objetos que já aparecem (estrela, balão); o DESAFIO EXTRA revela objetos NOVOS/inéditos** (ex.: cartola, gravata-borboleta) — dá um brinde extra e evita repetir. (Se a cartela nova vier com bolinhas/detalhes brancos, limpar por código — trocar o branco pela cor de base — pra não parecer furo/artefato.) Linhas ligadas via SVG (`<line>` por `createElementNS`, `viewBox 0 0 100 100`+`preserveAspectRatio=none`, pontos em %); o próximo número pulsa; tocar fora de ordem orienta "toque no número X"; anti-toque-duplo ignora o mesmo número em <380ms. **Ao CONCLUIR, o estado final é SÓ a imagem limpa** — as linhas e os pontos em volta somem (fade + remover do DOM); durante a ligação eles acendem, mas no fim ficam só a figura colorida (a professora pediu "sem as coisas em volta").
...
**FASE VOGAIS (letra inicial de vogais):** cada rodada mostra uma coisa do tema como imagem e a criança TOCA na vogal (bolha) com que o nome começa. As vogais são faladas foneticamente e pausadas (`vogalFalada`: A→"á", E→"é", I→"i", O→"ó", U→"úú"); a tela mostra a LETRA normal (A E I O U), a fala usa o som. Enunciados/dica/erro/gancho falam "vogal", não "letra"; a narração do enunciado NÃO entrega a resposta ("Ouça: <nome>. Toque na vogal com que começa!"). **No ACERTO, dizer "começa com a VOGAL <som>" (não "com o SOM")** — ex.: "arara começa com a vogal á" (a professora pediu). **As bolhas são as vogais como IMAGEM** (letras de circo geradas/recortadas = "bolha-imagem"; miolo do A/O transparente, bolinhas do E mantidas) — melhor que texto. **I e O são difíceis por tema — escolher palavras de nome claro e inequívoco** (mar: água-viva=A, estrela=E, ilha=I, ostra=O, uva=U; circo: A=arara, E=elefante, I=ioiô, O=óculos, U=urso). As 5 bolhas podem ser as próprias letras (texto grande em bolha colorida) — renderizam em qualquer navegador.
...
**⚠️ BOTÃO NARRADO (TRAVA DE ÁUDIO) — APOSENTADO (jul/2026, pedido do Marcos):** a trava que desabilitava o botão até a narração terminar **não se usa mais em nenhuma atividade** ("agora não quero mais a trava no botão"). O padrão atual é **voz NÃO-BLOQUEANTE** (ver MANUAL §"DOSE DE VOZ"): o botão aparece habilitado, a narração toca, e clicar avança na hora (a fila `falarDepois` emenda sem cortar o feedback). O texto abaixo fica só como REGISTRO histórico:
...
- **ARTIGO/PREPOSIÇÃO x GÊNERO DO NOME (do/da, o/a) — lição paga:** ao falar o nome de um bicho/objeto, o "do/da" (e "o/a") tem de casar com o gênero DELE. Erro cometido na fase da sombra: "a sombra **do** foca / **do** arara" (foca e arara são FEMININAS → "**da** foca", "**da** arara"). Guardar o gênero de cada item numa tabela (`ANIMGEN`, `g` em `FORMAS`) e montar o trecho por código (`nomeDoAnim(ch)` → "da foca"/"do leão"; `artG(g)` → "A"/"O"). NUNCA cravar "do "/"a " fixo antes de um nome variável. A auditoria (`auditar.py`, item 6b) varre "do <feminino>" e "da <masculino>".
- **NOMEAR O OBJETO no feedback (fase de forma):** ao acertar na fase de formas, falar o NOME do objeto, não "esse tem forma de X" — ex.: "**O presente** tem forma de quadrado!", "**A bola** tem forma de círculo!" (conecta objeto↔forma e soa natural). Requer `nome`+`g` em cada item de `FORMAS`.
...
**NÃO REPETIR O ALVO/RESPOSTA EM DESAFIOS SEGUIDOS (lição paga):** a mesma resposta não pode cair em desafios CONSECUTIVOS (ex.: fase de forma pedindo "quadrado→presente" e no desafio seguinte "quadrado→presente" de novo). Montar a sequência de alvos com `seqSemRepetir(lista,qtd)` (nunca dois iguais em seguida) em vez de `shuffleReal` puro. Como o alvo muda a cada desafio, o objeto-resposta também não repete. Vale para forma (alvo=forma), cor (alvo=cor) e afins.

**Fluxo:**
1. Claude entrega os prompts prontos numa página HTML com botão "Copiar". Cada prompt (cartela ÚNICA) DEVE pedir: itens em grade bem separados; fundo branco liso #FFFFFF sem textura/moldura/cartão; cartoon flat infantil, cores vivas; **SEM NENHUMA SOMBRA (nem projetada, nem elíptica de contato embaixo) — objetos flutuando no branco puro**; **contorno escuro e fechado em volta de cada objeto** (separa do fundo e do próprio branco interno, ex.: ovo); **sem partes brancas/cinza coladas na borda externa**; **objetos sólidos sem vãos internos vazados**; sem texto/letras/números. As CENAS (abertura/final) pedem retângulo deitado 3:2 preenchendo tudo (podem ter sombras normais, pois viram fundo inteiro). **As ILHAS/cenários das paradas seguem a regra de OBJETO (flutuando no branco, sem fundo/retângulo/cena atrás, sem texto) — NÃO são "cena" (senão o ChatGPT devolve retângulo com parede/chão/céu); ver §2. Podem vir agrupadas em cartelas (ex.: 6+5) e são recortadas por célula.**
...
- **Números como IMAGEM (cartela):** algarismos coloridos do tema (não botão). IA erra número — conferir cada um. "10" = juntar os 2 glifos (agrupar por gap, não por componente). Miolo do 0/4/6/8/9 transparente (região branca enclausurada); atenção: o miolo do 4 é pequeno e pode escapar do limiar. Números soltos e flutuando.
- **AUDITAR ANTES DE CADA publicação (não só na entrega):** rodar `_circo/auditar.py <index.html>` antes de TODO publish — checa tokens, sintaxe, JS/CSS moderno, funções duplicadas, **concordância (1 + feminino na fala)**, keyframes, onclick órfão. Só publicar se APROVADO. (Regra criada após publicar "um parada" por não ter auditado.)
- **Narração robusta (erra→acerta e o botão não libera):** a corrida `cancel()`→`speak()` buga o `onend` no celular. Detectar fim por poll de `speechSynthesis.speaking`; contador `_falaSeq` (nova fala invalida a antiga); limpar `_pollAtivo` no `pararFala`; atraso ~130ms + `resume()` antes do `speak()`; teto do gate ~9s. Testar ERRAR→ACERTAR.
- **Não tirar a palavra da lição por causa da voz:** se o TTS lê mal a palavra que É o conteúdo (ex.: "cor"), NÃO reescreva pra remover; garanta texto==fala, pegue a melhor voz pt-BR (natural/neural, tom pitch~1.04), e se preciso dê a pronúncia fonética só na fala. Lembrar do CACHE (`?v=N`).
- **Valor aleatório no texto E na fala → calcular UMA vez:** elogio/frase sorteada usada no `innerHTML` e no `falar()` precisa ser guardada numa var e reusada (`var el=elogio()`); chamar a função 2× faz a tela mostrar um e a voz falar outro (bug "a explicação fala diferente do texto").
