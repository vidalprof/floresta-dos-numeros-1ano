...
- **Pintura livre em `<canvas>`:** pincel gordinho e redondo (`lineWidth` ~22, `lineCap/lineJoin:"round"`; ponto = `arc(r=11)` no toque inicial, linha entre pontos no arrasto). Canvas com `width/height` fixos (600×440) + CSS `width:100%;height:auto` → mapear o toque por `(clientX-rect.left)/rect.width*canvas.width`. `touch-action:none` no canvas pra não rolar a página ao desenhar. Tratar mouse E touch (`ontouchstart/move/end/cancel`, com `preventDefault`).
...
- **Musiquinha com Web Audio (reaproveita `nota()`):** `setInterval` ~470ms tocando uma escala suave em `sine` (vol ~0.05) + oitava acima baixinho; botão liga/desliga. **Sempre `clearInterval` ao SAIR** (chamar `pararMusPinta()` no início de `telaMapa`, `telaFinal` e `telaPintar`) — senão o loop continua tocando na próxima tela. O áudio já está destravado aqui (a tela é aberta por clique).
- **DESENHO PARA COLORIR por cima (livro de colorir):** o ideal não é tela branca vazia — é um DESENHO de contorno pra criança pintar dentro. Gerar no ChatGPT um "coloring page" (só linhas pretas grossas, miolo branco, formas grandes/simples). Converter em LINHAS PRETAS TRANSPARENTES: `alpha = 255 - luminância` e `alpha=0` onde `lum>238` (branco→transparente), RGB tudo preto → PNG com miolo vazado e bordas suaves. Sobrepor esse PNG NO `<img class="pintadesenho">` ABSOLUTO em cima do `<canvas>` (mesma `.pintawrap` relativa), com `pointer-events:none` — a criança pinta no canvas por baixo (o toque atravessa) e as linhas ficam sempre visíveis. Canvas QUADRADO (600×600) pra casar com o desenho. "Limpar" repinta só o canvas (o desenho fica).
- Só `var`/`function`/`for`; sem arrow/template-literal. `setInterval`/`clearInterval`/`setTimeout`/`Math.PI` são permitidos. Conferir por screenshot desenhando algumas rabiscadas por código (`escolherCor(i)`+`linhaPinta(...)`).
