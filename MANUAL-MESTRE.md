...
**TRANSIÇÕES DE TELA SUAVES — SEM "VIRAR PÁGINA"/DESLIZAR (preferência da prof):** as telas/cards devem aparecer com **fade suave de opacidade**, nunca com deslize/`translateY` que dá sensação de "virar folha" a cada troca (cansa e distrai, ainda mais na pré-escola). Ou seja, a animação de surgimento do card é só `opacity 0→1` (curta, ~.35s), sem mover no eixo Y. (Ajusta a regra antiga de "cardSurge com deslize": manter suave, sem virada.)