...
- **⚠️ LIMITE HONESTO das ferramentas (não prometer demais):** Auditor + Testador pegam só **crash (erro de JS)** e **compatibilidade**. NÃO pegam **bug visual/comportamento** (ex.: popup que fica preso na tela), nem **pronúncia**, nem **pedagogia**. Esses precisam de **olho/ouvido humano**. Nunca dizer que o Testador garante "sem erros" — ele garante "sem crash e compatível". **Todo bug concreto novo → virar um CHECK dirigido** (regra "vira barreira"): quando dá pra reproduzir (como o popup), escrever um teste que dispara e confere o comportamento.
