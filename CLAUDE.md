...
> **🎨 (set/2026) O "NÍVEL GEMINI/CHATGPT" TEM CAMINHO PRONTO:** `gerar-imagens.yml` com `modelo=openai` (gpt-image-1: fundo transparente nativo + edita a base do mascote; ~US$0,04/figura em `medium`) — precisa do secret `OPENAI_API_KEY` que só o Marcos cola em Settings → Secrets (nunca pelo chat). Comparação medida de preço/qualidade e a fila de refazer do banco: **`_padrao/GERADORES-DE-IMAGEM.md`** + `_banco/AUDITORIA.md`. Portão novo **1c2 `_qa/duplicatas.py`**: a MESMA figura com dois nomes na mesma atividade reprova (foi assim que a letra I em pão era o H e o sol virava estrela).
