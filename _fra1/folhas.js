...
/* ============================================================
   A TECLA DO ESPAÇO QUEBROU — a casa das dez folhas.
   ...
   ⚠️ ESTE ARQUIVO NASCEU CLONADO da Letra que Muda Tudo (degrau 6). Tudo o que
   era CONTEÚDO de lá — os nomes das letras, a janela que troca de figura, a
   varredura por letra — foi ARRANCADO, não deixado inerte: código morto de
...
function f0(d){
  ...
  c.innerHTML =
    ...
    '<div class="cena"><div class="cpfila">' + '<div class="it" style="animation-delay:0.00s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/fr_bola.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.35s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/fr_sol.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.70s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/fr_pato.png?v=' + V + '" alt="">' + '' + '</div>' + '</div><div class="tecla">ESPAÇO</div></div>' +
...
function figurasDaFrase(figs){
  ...
  for(k = 0; k < figs.length; k++) tira.innerHTML += img(figs[k], "figfrase");
  return tira;
}
function ouvirFrase(chave, rot){
  var b = el("button", "bt azul ouvirped", rot || "Escutar a frase");
  b.onclick = function(){ sPasso(); falar("fr_" + chave); };
  return b;
}
...
/* 1, 2 e 3 — CORTE A FRASE (o bloco da tecla quebrada)
   ⚠️ AS TRÊS SEGUIDAS, subindo um degrau de cada vez: três palavras, quatro,
   cinco. A mecânica não muda — muda quantos cortes ela tem que achar, e isso é
   carga de memória, não conteúdo novo. Regra do Marcos: mecânica que repete vem
   em BLOCO, nunca espalhada, senão a criança sente que voltou. */
function f1(d, pi){ cortaFrase(d, pi, "e1_", ST.folha.p1, 0, "p1enun",
  "A tecla do espaço quebrou! Toque na letra que <b>começa</b> cada palavra nova."); }
function f2(d, pi){ cortaFrase(d, pi, "e2_", ST.folha.p2, 1, "p2enun",
  "Agora são <b>quatro</b> palavras grudadas. Ache todos os começos!"); }
function f3(d, pi){ cortaFrase(d, pi, "e3_", ST.folha.p3, 2, "p3enun",
  "<b>Cinco</b> palavras. Escute a frase antes de cortar."); }

function cortaFrase(d, pi, tag, L, nomeIdx, chaveEnun, texto){
  faixa(d, pi, NOMES[nomeIdx]);
  enunciado(d, pi, texto, chaveEnun);
  var n = nomeIdx + 1;
  for(var i = 0; i < L.length; i++){
    (function(chave, i){
      var id = tag + i, box = item(i + 1);
      var txt = FRASES[chave][0], figs = FRASES[chave][1];
      var cola = colada(txt), cortes = cortesDe(txt), pronto = !!ST.resp[id];
      registra(id, pi, txt);
      box.appendChild(figurasDaFrase(figs));
      box.appendChild(ouvirFrase(chave));
      var fila = el("div", "frasefila"), bts = [], achados = {}, quantos = 0, k;
        ...
        (function(k){
          ...
          b.onclick = function(){
            ...
            if(!cortavel || achados[k]){
              ...
              errou(id, "dica" + n + "_" + chave); return;
            }
            sTecla(); b.className = "lfr cortada";
            achados[k] = 1; quantos++;
            if(quantos === cortes.length){
              /* ⭐ O PRÊMIO É VER A FRASE INTEIRA SE SEPARAR e ouvi-la lida com
                 os espaços no lugar. É o momento em que o conceito aparece. */
              fila.className = "frasefila aberta";
              falar("fr_" + chave);
              setTimeout(function(){ acertou(id, "certo" + n + "_" + chave); }, 1400);
            }
          };
...
/* 4 — QUANTAS PALAVRAS TEM A FRASE? (das folhas d03, d05 e d07)
   ⭐ Aqui a frase já vem separada: o que se mede não é mais achar o espaço, é
   SABER QUE aquilo que está entre dois espaços é uma palavra — inclusive o "O"
   e o "A", que a criança de seis anos costuma grudar no vizinho porque são
   pequenos demais para parecerem palavra.
   ⚠️ Ela MARCA uma bolinha por palavra em vez de escolher um número pronto:
   marcar produz a quantidade um a um e deixa se corrigir. O "Pronto" é dela. */
function f4(d, pi){
  faixa(d, pi, NOMES[3]);
  enunciado(d, pi, "Pinte <b>uma bolinha</b> para cada palavra da frase. Depois toque em Pronto.", "p4enun");
  var L = ST.folha.p4;
  for(var i = 0; i < L.length; i++){
    (function(chave, i){
      var id = "e4_" + i, box = item(i + 1);
      var txt = FRASES[chave][0], ps = txt.split(" "), feito = !!ST.resp[id];
      registra(id, pi, String(ps.length));
      box.appendChild(figurasDaFrase(FRASES[chave][1]));
      box.appendChild(ouvirFrase(chave));
      var fr = el("div", "frasepalavras"), k;
      for(k = 0; k < ps.length; k++) fr.appendChild(el("span", "palfr", ps[k]));
      box.appendChild(fr);
      var fila = el("div", "bolinhas"), marc = feito ? ps.length : 0, bols = [];
      for(k = 0; k < 6; k++){
        (function(k){
          var b = el("button", "bolinha" + (feito && k < ps.length ? " cheia" : ""), "");
          ...
          b.onclick = function(){
            ...
            /* pinta/despinta SEMPRE da esquerda para a direita: a criança de 6
               anos não conta buracos no meio da fila */
            marc = (k < marc) ? k : k + 1;
      ...
      pr.onclick = function(){
        if(ST.resp[id]) return;
        if(marc === ps.length){ acertou(id, "certo4_" + chave); box.className = "item feito"; }
        else { sErro(); fila.className = "bolinhas erro";
               setTimeout(function(){ fila.className = "bolinhas"; }, 480);
               errou(id, "dica4_" + chave); }
      };
...
/* 5 e 6 — PONHA AS PALAVRAS NA ORDEM (das folhas d11 e d17)
   As palavras vêm soltas e fora de ordem, e agora a peça é a PALAVRA inteira.
   ⚠️ A folha 6 põe UMA PALAVRA A MAIS, de outra frase. Sem ela a última vaga se
   resolve por eliminação e a criança não precisa ler nada. */
function f5(d, pi){ montaFrase(d, pi, "e5_", ST.folha.p5, false, 4, "p5enun",
  "As palavras saíram fora de ordem. <b>Monte a frase.</b>"); }
function f6(d, pi){ montaFrase(d, pi, "e6_", ST.folha.p6, true, 5, "p6enun",
  "Agora <b>uma palavra sobra</b>! Monte a frase e deixe a intrusa de fora."); }

function montaFrase(d, pi, tag, L, comSobra, nomeIdx, chaveEnun, texto){
  faixa(d, pi, NOMES[nomeIdx]);
  enunciado(d, pi, texto, chaveEnun);
  var n = nomeIdx + 1;
  for(var i = 0; i < L.length; i++){
    (function(reg, i){
      var chave = comSobra ? reg[0] : reg, id = tag + i, box = item(i + 1);
      var txt = FRASES[chave][0], ps = txt.split(" "), pronto = !!ST.resp[id];
      registra(id, pi, txt);
      box.appendChild(figurasDaFrase(FRASES[chave][1]));
      box.appendChild(ouvirFrase(chave));
      var trilha = el("div", "vagasfr"), vagas = [], k;
      for(k = 0; k < ps.length; k++){
        var v = el("div", "vagafr" + (pronto ? " ok" : ""), pronto ? ps[k] : "");
        trilha.appendChild(v); vagas.push(v);
      }
      box.appendChild(trilha);
      /* o banco em ordem FIXA (o portão do jogador precisa de resultado
         estável), mas nunca na ordem da frase: a primeira palavra vai para o fim */
      var lista = ps.slice(0);
      if(comSobra) lista.push(reg[1]);
      lista = lista.slice(1).concat([lista[0]]);
      var banco = el("div", "bancofr"), posto = 0;
      lista.forEach(function(p2, j){
        var b = el("button", "palbt" + (pronto ? " usada" : ""), p2);
        b.setAttribute("data-qa", "pal-" + id + "-" + j);
        b.setAttribute("aria-label", "palavra " + p2);
        function poe(){
          if(ST.resp[id] || b.className.indexOf("usada") > -1) return;
          if(p2 !== ps[posto]){
            sErro(); b.className = "palbt erro";
            setTimeout(function(){ b.className = "palbt"; }, 460);
            errou(id, "dica" + n + "_" + chave); return;
          }
          sPasso();
          vagas[posto].className = "vagafr ok"; vagas[posto].textContent = p2;
          b.className = "palbt usada"; posto++;
          if(posto === ps.length){
            falar("fr_" + chave);
            setTimeout(function(){ acertou(id, "certo" + n + "_" + chave); }, 1300);
          }
        }
...
/* 7 — QUAL É A FRASE DA FIGURA? (da folha d17)
   As três frases falam da MESMA figura e mudam uma palavra só — o verbo ou o
   lugar. Quem lê só o começo erra, que é o mesmo hábito que a folha 4 do degrau
   6 desmontava, agora no tamanho da frase. */
function f7(d, pi){
  faixa(d, pi, NOMES[6]);
  enunciado(d, pi, "Olhe a figura e escolha a frase <b>certa</b>. Elas são parecidas!", "p7enun");
  var L = ST.folha.p7;
  for(var i = 0; i < L.length; i++){
    (function(reg, i){
      var chave = reg[0], id = "e7_" + i, box = item(i + 1);
      box.appendChild(figurasDaFrase(FRASES[chave][1]));
      var ops = reg.slice(0);
      var gira = i % 3;
      ops = ops.slice(gira).concat(ops.slice(0, gira));
      opcoes(box, pi, id, ops.map(function(c){
        return {v: c, rot: '<span class="frop">' + FRASES[c][0] + "</span>",
                aria: FRASES[c][0], fala: "fr_" + c};
      }), chave, "pintavel frbt", "certo7_" + chave, "dica7_" + chave);
      fechaItem(d, box, id);
    })(L[i], i);
  }
}

/* 8 — LIGUE A FIGURA À FRASE
   Três de uma vez: ela precisa segurar três frases na cabeça e decidir qual vai
   com qual. Mesmo conteúdo, carga maior. */
function f8(d, pi){
  faixa(d, pi, NOMES[7]);
  ...
  for(var i = 0; i < L.length; i++){
    var g = L[i], cx = el("div", "");
    montaLigar(cx, pi, "e8g" + i, g.map(function(c){
...
/* 9 — ESCREVA A PALAVRA QUE FALTA (o degrau mais alto: sem opções)
   ⭐ AS DUAS PORTAS, regra da casa: o teclado da tela E o teclado de verdade.
   ⚠️ E só entram palavras CURTAS e já vistas no caderno (SOL, OVO, MEL, UVA,
   BOLA): o degrau aqui é a FRASE, não a ortografia. Cobrar uma palavra comprida
   trocaria o que a folha mede. */
function f9(d, pi){
  faixa(d, pi, NOMES[8]);
  enunciado(d, pi, "Escute a frase e <b>escreva a palavra que falta</b>.", "p9enun");
  var L = ST.folha.p9;
  for(var i = 0; i < L.length; i++){
    (function(reg, i){
      var chave = reg[0], onde = reg[1], id = "e9_" + i, box = item(i + 1);
      var ps = FRASES[chave][0].split(" "), certa = ps[onde], feito = !!ST.resp[id];
      registra(id, pi, certa);
      box.appendChild(figurasDaFrase(FRASES[chave][1]));
      box.appendChild(ouvirFrase(chave));
      var fr = el("div", "frasepalavras"), k, q = null;
      for(k = 0; k < ps.length; k++){
        if(k === onde){
          ...
          fr.appendChild(q);
        } else fr.appendChild(el("span", "palfr", ps[k]));
      }
      box.appendChild(fr);
      fechaItem(d, box, id);
    })(L[i], i);
...
/* 10 — O MURAL DAS FRASES (o fecho, e ele GUARDA o que ela fez)
   ⭐ Regra 11 da pesquisa: a tela final mostra o que a CRIANÇA fez. Cada frase
   escolhida entra no mural com os espaços no lugar — o retrato do conserto que
   ela fez na tecla quebrada. */
function f10(d, pi){
  faixa(d, pi, NOMES[9]);
  enunciado(d, pi, "Toque nas frases que você consertou e quer no <b>seu mural</b>.", "p10enun");
  var L = ST.folha.p10, mural = el("div", "muralfr");
  for(var i = 0; i < L.length; i++){
    (function(chave, i){
      var id = "e10_" + i;
      registra(id, pi, chave);
      var c = el("button", "cartafr",
        img(FRASES[chave][1][0], "figop") + '<span class="frmural">' + FRASES[chave][0] + "</span>");
      c.setAttribute("data-qa", "mural-" + id);
      c.setAttribute("aria-label", FRASES[chave][0]);
      c.onclick = function(){
        if(ST.resp[id]) return;
        sPasso(); falar("fr_" + chave);
        c.className = "cartafr escolhido";
        acertou(id, "certo10_" + chave);
      };
      if(ST.resp[id]) c.className = "cartafr escolhido";
      mural.appendChild(c);
    })(L[i], i);
  }
  d.appendChild(mural);
...
function idsDaPagina(pi){
  /* ⚠️⚠️ OS IDS TÊM QUE BATER COM O QUE AS FOLHAS GRAVAM (prefixo `e`, deste
  ...
  if(pi === 1) for(i = 0; i < F.p1.length; i++) ids.push("e1_" + i);
  if(pi === 2) for(i = 0; i < F.p2.length; i++) ids.push("e2_" + i);
  if(pi === 3) for(i = 0; i < F.p3.length; i++) ids.push("e3_" + i);
  if(pi === 4) for(i = 0; i < F.p4.length; i++) ids.push("e4_" + i);
  if(pi === 5) for(i = 0; i < F.p5.length; i++) ids.push("e5_" + i);
  if(pi === 6) for(i = 0; i < F.p6.length; i++) ids.push("e6_" + i);
  if(pi === 7) for(i = 0; i < F.p7.length; i++) ids.push("e7_" + i);
  if(pi === 8) for(i = 0; i < F.p8.length; i++) for(k = 0; k < F.p8[i].length; k++)
    ids.push("l8e8g" + i + "_" + F.p8[i][k]);
  if(pi === 9) for(i = 0; i < F.p9.length; i++) ids.push("e9_" + i);
  if(pi === 10) for(i = 0; i < F.p10.length; i++) ids.push("e10_" + i);
  return ids;
}
...
function fim(){
  ...
  for(ke = 0; ke < 3; ke++)
    /* ⚠️ O SELO DA MEDALHA CHAMA-SE `mo_selo`, NÃO `mo_estrela` — e isto é um
       ...
       de palavra do pote. */
    est += '<img src="img/fr_selo' + (ke < cheias ? "" : "_off") + '.png?v=' + VIMG + '" alt="" draggable="false">';
  ...
  else txt = "Você começou a descobrir para que serve o espaço entre as palavras!";
...
var OBJETIVOS = [
  {n: "Achar onde uma palavra acaba", f: [1, 2, 3],
   ok: "acha onde cada palavra começa dentro de uma frase escrita sem espaços",
   nao: "ainda não separa uma palavra da outra — a frase é um bloco só"},
  {n: "Contar as palavras da frase", f: [4],
   ok: "conta quantas palavras a frase tem, contando também o O e o A",
   nao: "ainda gruda as palavras pequenas (o, a, na, da) na palavra vizinha"},
  {n: "Pôr as palavras na ordem", f: [5, 6],
   ok: "monta a frase com as palavras fora de ordem, e descarta a que sobra",
   nao: "ainda troca a ordem das palavras ou usa a palavra que era de outra frase"},
  {n: "Ler a frase inteira", f: [7],
   ok: "escolhe a frase certa entre três que mudam uma palavra só",
   nao: "ainda decide pelo começo da frase; vale ler as três em voz alta e comparar"},
  {n: "Casar frase e figura", f: [8],
   ok: "liga cada figura à frase que fala dela",
   nao: "ainda se perde quando precisa segurar três frases de uma vez"},
  {n: "Escrever a palavra", f: [9],
   ok: "escreve sozinha a palavra que falta na frase, sem opções na tela",
...
function abreRelatorio(){
  ...
  if(domina.length && !retomar.length && !naoAlcancou.length)
    parecer += "sabe onde uma palavra acaba e a outra começa, em todos os degraus avaliados: " +
      domina.join("; ") + ".";
  else if(domina.length)
    parecer += "já " + domina.join("; ") + ". Ainda precisa retomar: " + retomar.join(", ") + ".";
  else
    parecer += "ainda está descobrindo para que serve o ESPAÇO em branco. Nenhum objetivo " +
      "chegou a 75% de acerto de primeira — vale retomar escrevendo uma frase na lousa SEM " +
      "espaços e lendo junto com ela, batendo palma a cada palavra, antes de voltar à tela.";
