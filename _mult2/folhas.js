...
function monta(){
  ...
  var caps = [f0, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13,
              f14, f15, f16, f17, f18, f19, f20, f21], i;
...
function fim(){
  ...
  for(ke = 0; ke < 3; ke++)
    est += '<img src="img/ho_estrela' + (ke < cheias ? "" : "_off") + '.png?v=' + VIMG + '" alt="" draggable="false">';
...
var OBJETIVOS = [
  {n: "Ler a cena em grupos iguais", f: [1, 2, 3],
  ...
  {n: "Da soma de parcelas iguais para a conta de vezes", f: [4, 5, 6, 7],
  ...
  {n: "Escrever o total e a tabela", f: [8, 9, 10],
   ok: "escreve o total de próprio punho, pela soma e pela multiplicação, e completa a tabela",
   nao: "ainda reconhece a conta certa, mas não produz o resultado sozinha"},
  {n: "As quatro tabuadas do ano", f: [11, 12, 13, 14],
   ok: "responde as tabuadas do 2, do 3, do 4 e do 5 sem precisar recomeçar a soma toda vez",
   nao: "ainda soma de novo a cada pergunta, sem os fatos na memória"},
  {n: "Contar de tanto em tanto", f: [15, 16],
   ok: "conta de 2 em 2, de 3 em 3, de 4 em 4 e de 5 em 5 sem perder a fila",
   nao: "ainda conta de um em um para chegar ao próximo número da sequência"},
  {n: "Dobro, metade e triplo", f: [17, 18, 19],
   ok: "acha o dobro e o triplo de um número e reparte ao meio, entendendo que dobro é o mesmo tanto duas vezes",
   nao: "ainda trata dobro, metade e triplo como palavras soltas, sem ligar com a conta de vezes"},
  {n: "Resolver problemas escritos", f: [20],
   ok: "lê o problema, escolhe a multiplicação e escreve a resposta",
   nao: "ainda depende da figura para decidir que conta fazer"},
  {n: "Registrar o que aprendeu", f: [21],
   ok: "monta o cartaz da horta com as contas que já domina",
...
function f0(d){
  ...
  var c = el("div", "capa"), nome = "APRENDENDO A MULTIPLICAÇÃO, O DOBRO, A METADE E O TRIPLO", k, letras = "", pos = 0;
  ...
  c.innerHTML =
    ...
    '<div class="sub">Matemática &middot; 2º ano &middot; vinte e uma folhas de multiplicação</div>' +
    '<div class="cena">' + '<div class="it" style="animation-delay:0.00s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ho_morango.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.35s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ho_laranja.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:0.70s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ho_maca.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:1.05s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ho_pera.png?v=' + V + '" alt="">' + '' + '</div>' + '<div class="it" style="animation-delay:1.40s">' + '<img class="capfig" draggable="false" onload="naoAmplia(this)" src="img/ho_melancia.png?v=' + V + '" alt="">' + '' + '</div>' + '</div><div class="terra"></div>' +
...
/* 11 a 15 — AS CINCO MÁQUINAS (papel d06: "2 X" ligado a 12, 7, 15, 6, 9; e a
   ...
   ⚠️ São exatamente as QUATRO da rede no 2º ano: 2, 3, 4 e 5. A do 10 existe no
      caderno do 3º ano e NÃO entra aqui — o 10 é do ano seguinte nesta rede. */
...
function f12(d, pi){ maquina(d, pi, ST.folha.p12, 3, "p12"); }
function f13(d, pi){ maquina(d, pi, ST.folha.p13, 4, "p13"); }
function f14(d, pi){ maquina(d, pi, ST.folha.p14, 5, "p14"); }

/* 15 e 16 — CONTAR DE TANTO EM TANTO (papel d28, segunda metade: "Descubra o
...
/* 17, 18 e 19 — DOBRO, METADE e TRIPLO
   ⚠️⚠️ ESTE BLOCO NÃO EXISTE NO CADERNO DO 3º ANO. Ele é a SEGUNDA habilidade
      do 2º ano nesta rede, verbatim: *"Resolver e elaborar problemas envolvendo
      dobro, metade, triplo e terça parte, com o suporte de imagens ou material
      manipulável"*.
   ⚠️ E ele entra DECLARADO como bloco vindo do currículo, não da colheita: das
      quarenta folhas de papel lidas, UMA só pede isto (a d31, "O triplo de 8
      maçãs são…"). Quando o papel para num degrau abaixo, quem manda é a rede.
   ⭐ A cena é a MESMA do caderno inteiro — grupos iguais na prateleira — e isso
      é de propósito: dobro não é conta nova, é a conta de vezes que ela passou
      catorze folhas aprendendo, com outro nome. Dobro = dois grupos iguais;
      triplo = três; metade = olhar um grupo só dos dois.
   ⚠️ A TERÇA PARTE, que a habilidade também cita, NÃO está aqui, e fica dito:
      ela pede repartir em três, que é divisão, e o caderno inteiro é de
      multiplicação. Entra no caderno de divisão do ano. */
function f17(d, pi){
  faixa(d, pi, NOMES[16]);
  enunciado(d, pi, "O vovô <b>dobrou</b> a colheita. Quanto ficou?", "p17enun");
  var L = ST.folha.p17;
  for(var i = 0; i < L.length; i++){
    (function(it, i){
      var id = "n17_" + i, box = item(i + 1);
      box.appendChild(el("div", "pedido", "o <b>dobro</b> de <b>" + it.n + "</b>"));
      box.appendChild(prateleira(it.w, 2, it.n));
      opcoes(box, pi, id, opsNum(it.op), String(2 * it.n), "figbt",
             "certodobro_" + it.n, "dicadobro_" + it.n);
      fechaItem(d, box, id);
    })(L[i], i);
  }
}
...
function f18(d, pi){
  faixa(d, pi, NOMES[17]);
  enunciado(d, pi, "Agora ele repartiu em <b>duas partes iguais</b>. Quanto ficou em cada uma?", "p18enun");
  var L = ST.folha.p18;
  for(var i = 0; i < L.length; i++){
    (function(it, i){
      var id = "n18_" + i, box = item(i + 1);
      box.appendChild(el("div", "pedido", "a <b>metade</b> de <b>" + (2 * it.n) + "</b>"));
      box.appendChild(prateleira(it.w, 2, it.n));
      opcoes(box, pi, id, opsNum(it.op), String(it.n), "figbt",
             "certometade_" + it.n, "dicametade_" + it.n);
      fechaItem(d, box, id);
    })(L[i], i);
  }
}

/* 19 — O TRIPLO, e aqui ela ESCREVE (papel d31: "O triplo de 8 maçãs são…", que
   é uma linha para preencher, não uma opção para marcar). */
function f19(d, pi){
  faixa(d, pi, NOMES[18]);
  enunciado(d, pi, "<b>Três</b> canteiros com o mesmo tanto. Escreva o <b>triplo</b>.", "p19enun");
  var L = ST.folha.p19;
  for(var i = 0; i < L.length; i++){
    (function(it, i){
      var id = "n19_" + i, box = item(i + 1), tot = String(3 * it.n);
      registra(id, pi, tot);
      box.appendChild(el("div", "pedido", "o <b>triplo</b> de <b>" + it.n + "</b>"));
      box.appendChild(prateleira(it.w, 3, it.n));
      var lin = el("div", "contalin");
      lin.appendChild(el("span", "conta", "o triplo de " + it.n + " ="));
      var q = el("div", "sq vaga" + (ST.resp[id] ? " ok" : ""), ST.resp[id] ? tot : "");
      q.setAttribute("data-qa", "esc-" + id);
      q.setAttribute("role", "button"); q.setAttribute("tabindex", "0");
      q.setAttribute("aria-label", "escrever o triplo");
      q.onclick = function(){
        if(ST.resp[id]) return;
        ativa(q, tot, id, "certotriplo_" + it.n, "dicatriplo_" + it.n);
      };
      lin.appendChild(q);
      lin.appendChild(botaoSom("Ouvir a conta", function(){ falar("leitura_3x" + it.n); }));
      box.appendChild(lin);
      fechaItem(d, box, id);
    })(L[i], i);
  }
}
/* 20 — OS PROBLEMAS DA HORTA (papel d26: "Em um pacote há 9 balas. Quantas
   balas há em 3 pacotes?"; d38: "Cada bandeja tem 6 brigadeiros…"; d32: "Nasceu
   1 flor em cada vaso. Quantas flores há nos vasos?").
   ⚠️ O ENUNCIADO É FALADO, sempre. No 2º ano MUITA criança ainda lê devagar, e
   ...
   ⚠️ Os oito problemas foram REESCRITOS para este caderno: os do 3º ano usavam
      o fator 10 em quatro dos oito, e o 10 é do ano seguinte nesta rede. */
...
function f20(d, pi){ problemas(d, pi, ST.folha.p20, "prob20"); }

/* 21 — O CARTAZ DA HORTA (o fecho com gancho)
   ⭐ A criança escolhe as contas que ela quer pendurar no cartaz da horta, e
...
function f21(d, pi){
  faixa(d, pi, NOMES[20]);
  enunciado(d, pi, "Toque nas contas que você quer no <b>seu cartaz</b>. Ele fica guardado no fim.", "p21enun");
  var L = ST.folha.p21;
  var mural = el("div", "mural");
  for(var i = 0; i < L.length; i++){
    (function(it, i){
      var id = "n21_" + i, box = item(0), k = "c" + it.q + "x" + it.n;
      registra(id, pi, k);
      ...
      b.onclick = function(){
        ...
        acertou(id, "certo21_" + it.q + "x" + it.n);
      };
