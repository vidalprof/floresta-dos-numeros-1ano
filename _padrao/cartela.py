...
def e_cena(x):
    ...
    nome = x.get("nome", "")
    # ⚠️ AQUI MORAVA UM `or x.get("modelo") == "pollinations"`, e ele
    #    confundia DUAS coisas diferentes: "de graca" e "cena larga". Desde que
    #    o caminho gratis passou a RECORTAR (o `lote=` do gerar-imagens.yml:
    #    Pollinations desenha, rembg tira o fundo), uma peca pode ser gratis e
    #    continuar sendo peca. Com a linha antiga, um lote de 23 pecas aparecia
    #    como "23 cenas largas" e o planejador nao tinha o que planejar.
    #    O dinheiro quem conta e o `_qa/cartela.py`, olhando o `modelo`.
...
def cortar(folha, nomes, dest, larg=520):
    ...
    # ⚠️ LICAO PAGA (ago/2026, na cartela da Oficina da Lina): "os N maiores
    #    componentes" perde a PECA COM PARTES SOLTAS. O desenho do vento tem a
    #    arvore, tres espirais e duas folhas voando — sete componentes para UMA
    #    peca. O corte ficou com a arvore e uma espiral; as outras cinco partes
    #    foram para o lixo como se fossem sujeira. E o pior: sai bonito, so que
    #    faltando — ninguem ve o que nao esta la.
    #    A cura e agrupar por CELULA: primeiro descobre onde estao as pecas
    #    (faixas de linha e de coluna com tinta), depois todo componente que
    #    cair dentro da mesma celula e a MESMA peca.
    def _faixas(v, minimo):
        out, ini = [], None
    ...
    celulas = []
    for (y0, y1) in _faixas(mask.sum(axis=1) > W * 0.004, int(H * 0.03)):
        faixa = mask[y0:y1]
        for (x0, x1) in _faixas(faixa.sum(axis=0) > (y1 - y0) * 0.004,
                                int(W * 0.03)):
            celulas.append((y0, y1, x0, x1))
    if len(celulas) == len(nomes):
        # cada celula vira um "componente" so, com todas as partes dentro dela
        junto = np.zeros_like(lab)
        for k, (y0, y1, x0, x1) in enumerate(celulas, start=1):
            pedaco = np.zeros_like(mask)
            pedaco[y0:y1, x0:x1] = mask[y0:y1, x0:x1]
            junto[pedaco] = k
        lab, n = junto, len(celulas)
        areas = ndimage.sum(np.ones_like(lab), lab, index=range(1, n + 1))
        minA = 0
    ...
    for nome, (cid, cy, cx, y0, y1, x0, x1) in zip(nomes, ordem):
        # ⚠️ LICAO PAGA (ago/2026, na cartela da identidade da Lina):
        #    `binary_fill_holes` tapa TODO buraco interno — e o vao da fita da
        #    medalha e o espaco entre as correntes da placa sao buracos
        #    LEGITIMOS: e por ali que se ve o fundo. Tapados, viraram manchas
        #    PRETAS solidas no meio da peca. O buraco que precisa ser fechado e
        #    o pequeno (o pixel que o limiar comeu dentro do cabelo); o grande e
        #    vazado de verdade. Entao so os PEQUENOS se fecham.
        base = (lab == cid)
        cheio = ndimage.binary_fill_holes(base)
        buracos, nb = ndimage.label(cheio & ~base)
        m = base.copy()
        if nb:
            areaP = ndimage.sum(np.ones_like(buracos), buracos,
                                index=range(1, nb + 1))
            limite = 0.02 * base.sum()          # 2% da peca
            for k in range(nb):
                if areaP[k] <= limite:
                    m |= (buracos == k + 1)
        ...
        # ⚠️ E O BRILHO (a lampada acesa, o fogo, a lanterna): o halo amarelo se
        #    dissolve no preto da folha, entao qualquer mascara DURA recorta um
        #    anel preto em volta da peca — que aparece feio sobre o fundo claro
        #    da atividade. Na zona escura, o alfa segue o BRILHO do pixel: o
        #    quase-preto some, o meio-tom fica meio transparente. Dentro da peca
        #    nada muda, porque ali o pixel e claro.
        # ⚠️ E SO NA FRANJA DA BORDA. Aplicado na peca inteira, este mesmo
        #    alfa come o ESCURO DE DENTRO: o cabelo preto sob o bone das seis
        #    criancas virou uma faixa desbotada, porque cabelo escuro e
        #    pixel escuro igual ao fundo. O halo do brilho vive na BORDA;
        #    entao so a borda e tratada, e o miolo fica opaco.
        luz = arr[yy0:yy1, xx0:xx1].max(axis=2).astype(np.float32)
        suave = np.clip((luz - 8.0) / 42.0, 0.0, 1.0) * 255.0
        miolo = ndimage.binary_erosion(a > 128, np.ones((3, 3)), iterations=3)
        suave[miolo] = 255.0
        a = np.minimum(a.astype(np.float32), suave).astype(np.uint8)
