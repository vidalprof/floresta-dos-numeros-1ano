...
|---|---|---|---|
| **Caça-palavras** | `_naveg` "CAÇA-PALAVRAS DO MAR" · `_mapa` | reconhecer a forma escrita de um termo novo; alívio entre fases pesadas | célula em **`(100/N)%` + `box-sizing:border-box`** (com px fixo cabem 10 numa grade de 9 → *"TROCA e o A em outra linha"*); **diagonal só se o enunciado avisar**; célula conquistada **trava**; conferir `mark` **OU** `ok` (palavra que cruza outra nunca fechava); publicar `data-qa` senão o auditor dá "PRESO"; **escrever a palavra ACENTUADA** ("ROÇA", "FÁBRICA", "ÁRVORE") — a grade dobra o acento sozinha (`_gl`), o chip mostra e a VOZ lê com acento; palavra sem acento faz a voz ler outra palavra (Marcos ouviu "roca" por "roça", set/2026 → portão em `montar.py`) |
...
| **Memória** | `_mapa` (carta grande, verso de arte, virada 3D) | par **conceitual** (causa↔efeito, palavra↔imagem) | carta fluida **≥ 130×88px**; verso de **arte de IA**, nunca retângulo liso; `rotateY` com queda para troca-de-face no Chrome 109; em tela baixa encolhe a LETRA, nunca a carta; som de virar e de par; **a arte de IA vai no campo `img`** (o motor lê pelo `imgEl`) — o `fig` do exemplo é só o desenho EMBUTIDO da peça; trocar um pelo outro deixa a carta mostrando **"undefined"** (pago no RIGHT NOW, ago/2026) ⭐ (set/2026) **memória de SONS**: com `som:true` no par, a segunda carta é só um alto-falante grande — virar toca a voz (`vozsen`/`voz`/`pal`) e a criança casa o que **ouve** com o que **vê** (EdiLIM "memoria de sonidos"); portão: carta-som sem voz reprova |
...
| **Bancada de potes (repartir × medir)** | `_div3` “Reparta em partes iguais” (peças `reparte` + `montaReparte`, folha viva) | os DOIS significados da divisão com a mesma mão: com o tamanho do pote dado conta-se POTES (medida), com o número de potes dado contam-se PEÇAS (repartição) | ⭐ **na MEDIDA o pote nasce um de cada vez** — com todos desenhados bastava contá-los e a pergunta *“quantos potes você precisou?”* se respondia sozinha; o pote publica **`pote-<id>-<j>`** (pelo ID DO ITEM, nunca pelo número da folha: três problemas na mesma folha publicavam a MESMA chave e o jogador clicava sempre no primeiro); a peça publica **`item-<id>-<n>`**; a resposta declarada é a distribuição INTEIRA, `rep:<potes>x<cada>`, e não um toque só — com a forma de dois toques do motor o jogador da banca dava o item por fechado na primeira peça e SEIS folhas apareciam como *“não fecha nem com a resposta certa”*; pote cheio **recusa** e chama a dica. Família “REPARTE” no `_qa/joga_folha.js`, escrita no mesmo commit |
| **Quociente parcial** | `_divisao` "PEDAÇO POR PEDAÇO" (peça `quociente-parcial`) | a ponte do 5º ano p/ o algoritmo: tirar do dividendo pedaços conhecidos (×10, ×5, ×2, ×1) e SOMAR | o **histórico dos pedaços fica à vista** (a conta visível é o coração — senão vira algoritmo cego); dados `{n,d}`, `n` até ~999, aceita resto; `data-qa` no maior pedaço que cabe; fecha por `mostraBanner`; mostra `N ÷ d = quociente [e sobra r]`. Origem: pesquisa set/2026 (partial quotients keep the math visible) |
...
| **caca-palavras** | achar a palavra escondida na grade — e, ao achar, **ver a figura e ouvir a palavra** (JClic: o caça-palavras que ensina ao achar) | célula em **porcentagem**, nunca px (grade de 9 quebra a linha em 320px); diagonal só se o enunciado avisar; palavra **sem acento** na grade e certa no chip/voz. ⭐ (set/2026) `FIG` = figura por palavra: no modo lista ela aparece **ao achar** (forma + sentido + som no momento do acerto); com `MODO="figuras"` (EdiLIM) a lista mostra só a figura com alto-falante e a criança tem que PENSAR a palavra — nunca pista que contém a própria palavra |
| **cruzadinha** | escrever a palavra da pista casa por casa, com as **duas portas** (teclado da tela e o de verdade) | resposta **sem acento** em `p` e a forma certa em `ac`; `document.onkeydown` com guarda `!t.parentNode`. ⭐ (set/2026) **a pista tem alto-falante** (`data-voz` = a pista limpa; regra 8 da pesquisa, JClic 1º ano) e, ao fechar a palavra, a voz diz a palavra **inteira** (separou → junta) |
| **relampago** | responder de cabeça contra o relógio, sem dica no meio e **sem castigo** no fim | nunca a palavra "errou": a certa acende e o que faltou volta como "treinar de novo"; o fim chama `fimDaPeca()` (senão é BECO). ⭐ (set/2026) **fecha com UMA pergunta de reflexão** (`.reflexao`, dita pelo mascote — Topmarks): velocidade sem reflexão ensina só a ser rápido |
...
| **pintar-desenho** | a única em que a criança **produz** em vez de responder | **não tem certo nem errado**: nenhum som de tropeço, nenhuma marca de recusa. O peixe pode ser roxo. O que se mede é AUTORIA. ⭐ (set/2026, regra 11) ao fechar chama **`guardaArtefato`** (SVG com as cores dela) — a obra vai para a galeria da tela final do motor; toda peça de PRODUÇÃO tem que chamar (`_qa/dinamicas.py` reprova) |
| **pintar-canvas** | o pintar do **Circo do Teo**: balde de tinta em **canvas** sobre uma IMAGEM de contorno (`img/<img>.png`), flood-fill parando nas linhas pretas | também **não tem certo nem errado**. A figura é ARTE DE IA (não vetor). ⚠️ a `.tela` tem que ROLAR por dentro (`overflow-y:auto`) — na tela baixa o canvas+paleta+botões não cabem e o portão leiaute só aceita a dobra se a própria tela rolar. Reserva `img:""` desenha um contorno no ctx para a bancada. ⭐ (set/2026, regra 11) `guardaArtefato` com o `toDataURL` do canvas dentro de `try` — canvas "manchado" (figura de outra origem / `file://`) estoura e a foto é pulada em silêncio |
...
| **tracar-letra** | refazer o movimento da letra, sempre no mesmo sentido | sair da linha **não pune** — som de retorno, nunca de tropeço; o dedo da criança treme. E conferir por POSIÇÃO, não por identidade do elemento. ⭐ (set/2026, regra 11) ao completar chama `guardaArtefato` com a letra em SVG (o caminho do fantasma pintado) |
...
| **caixas-de-som** | **o SOM isolado**: uma caixinha por som, e ela empurra uma ficha para cada um, dizendo a palavra devagar. É o degrau que a sílaba não alcança — e o preditor mais forte de leitura no 1º ano | a **letra não pode aparecer durante o preenchimento**: viraria ditado, que é outra coisa e vem depois. Primeiro o som; a letra só no fim, dentro das caixas. E `letras` **não é uma por caixa** — o "CH" de CHÁ é UM som e DUAS letras, e é isso que a peça mostra: por isso ela não pode ser feita contando letras. ⭐ (set/2026) **separou → JUNTA**: quando a palavra fecha, a seta corre embaixo das caixas e a voz diz a palavra **inteira, de uma vez** (NCIL: "deslize o dedo pela seta e leia a palavra"). Segmentar sem voltar a juntar é meio caminho |
| **rima** (ago/2026) | **o par por SOM do fim**: um tabuleiro de cartas viradas para cima; a criança toca em duas que **rimam** (PATO–GATO). Degrau **onset-rime** da escada fonológica — habilidade própria, o 3º tema mais pedido nas pesquisas (33 menções) e o gap que o nosso leque tinha | o par é por SOM, **nunca por posição** → tabuleiro **embaralhado** (`baguncar`); **alto-falante em toda carta** (classe `.ptxt`), a rima é do ouvido; erro não pune e o **andaime cresce** (escute o fim → o par pisca → o par acende); **duas portas** (toque e mouse) + guarda do clique fantasma; os dois lados do par publicam a **mesma `data-qa`** (o auditor fecha a fase). Portão: `_qa/dinamicas.py` (regra "rima") ⭐ (set/2026) **tocar a carta DIZ a palavra** (`fala()` em `marca`): o alto-falante é um segundo gesto; a rima é do ouvido, então a carta escolhida fala sozinha |
| **montar-frase** | pôr cada palavra na vaga com a **forma** da classe dela (artigo, nome, ação…) — toque, arrasto ou teclado | a forma é distribuída pelas categorias **da rodada**, nunca pelo nome (o 9º ano de inglês perdeu todas as formas por isso); o clique que fecha um arrasto não conta erro de novo (`ultimaSolta`); revela ANTES da dica no 3º degrau. ⭐ (set/2026) **a frase montada é LIDA inteira** (`f` da rodada, gravada pelo `montar.py`) — separou → junta |
| **ler-e-fazer** (set/2026) | a instrução vem **ESCRITA** e a criança **executa na cena** ("toque no pão"; depois dois passos em ordem) — ler é o conteúdo (TYMTR "read and do") | o pedido mora em `.tarefa`, **nunca no balão** (o motor narra o balão e leria pela criança); a **voz é o 2º degrau** do andaime (1º erro: aparece o alto-falante; 2º: a peça lê; 3º: a zona pisca; 4º: executa e segue); zona = figura (grade de bits, clone do `achar-na-cena`); passo da vez com `data-qa` |
| **marcar-varias** (set/2026) | uma pergunta com **2 ou 3 certas** entre 4–6 opções; a criança marca **todas** as que servem (EdiLIM "resposta múltipla") | a rodada fecha **sozinha** quando todas as certas estão marcadas (sem botão "pronto": o gesto é só tocar, e o jogador fecha pelo `data-qa`); marcar errada responde **na hora** (som grave, treme, sai do alcance) e o andaime cresce (dica → as que faltam piscam → revela); placar "faltam N" em texto, nunca só cor; `data-qa` entra em toda certa e **sai** da marcada |
| **escada-de-palavras** (set/2026) | MALA → MOLA → BOLA → BOLO: a cada degrau **uma letra muda** e nasce outra palavra (NCIL *word ladder*); dois toques por degrau — qual letra muda, qual letra entra | degrau tem que mudar **exatamente uma** letra, mesmo tamanho, sem acento; a palavra nova vem como **figura + alto-falante + pista**, nunca escrita (senão vira copiar); **duas portas** (a letra digitada vale no passo 2); quando troca, a **voz diz a palavra nova** (mudou um som, mudou a palavra); andaime: diga as duas devagar → a letra pisca → revela |
| **som-inicial** (ago/2026) | **a casa do som do começo**: casas rotuladas pela letra (M, S…), uma esteira de palavras; a criança manda cada palavra para a casa do som com que ela **começa** (MALA→M, SAPO→S). É onde a criança trava para decodificar; agrupar por som inicial é de alto valor (pesquisa ago/2026) | agrupar por **SOM**, não por categoria de sentido (isso é `classificar`); cartas **embaralhadas** (`baguncar`); **alto-falante** em carta e casa (`.ptxt`), a casa fala **o SOM** ("o som M"), nunca o nome da letra ("ême"); a carta vem **sem a letra inicial destacada** (entregaria a resposta sem ouvir); **1ª rodada por som contínuo** (M, S, F…); erro não pune, **andaime cresce** (escute o começo → a casa acende → revela); carta e casa publicam a **mesma `data-qa`**. ⭐ (set/2026) a casa aceita **`img` = figura-âncora** (a MÃO em cima do M — NCIL: "associe uma figura ao som-alvo"; é o que a criança lembra ao ver a letra); e **duas casas que se confundem** no ouvido (B/V, F/V, T/D, M/N, P/B) ou no olho (B/D, P/Q) **não vão na mesma rodada** antes de cada uma ter aparecido sozinha — o portão avisa. Portão: `_qa/dinamicas.py` (regra "som inicial") |
| **bater-silabas** | **análise**, a outra metade: BOLO = BO + LO. A criança **bate uma vez por pedaço** (palma / mão no queixo) | **não desenhar os lugares prontos** — com 3 casinhas na tela ela conta as casinhas, não a palavra, e a resposta está dada; tem que dar para **Apagar** (contar errado no meio não prende); 2º degrau do andaime é pelo **OUVIDO** (a peça bate junto), só o 3º escreve; **barra de espaço** bate e **Enter** responde (as duas portas); ⚠️ **CADEADO DE RE-ENTRADA** (`travando`): a comemoração do acerto dura segundos (uma sílaba por vez) e a criança impaciente toca **Pronto de novo** — sem o cadeado, `confere`/`acerta` roda 2× e a peça **trava** (Marcos pegou no ELEFANTE, a palavra mais longa = a maior janela). O gate `_qa/dinamicas.py` exige o `travando`. ⭐ **SOM CONTÍNUO ANTES DE PARADA** (set/2026, `COMO-FAZER-MELHORIAS-2026-08.md` §1): a PRIMEIRA palavra da mecânica (vale para `juntar-silabas` e `caixas-de-som` também) abre com som que se estica — M, S, F, V, N, L, R, Z, J ou vogal (MEL, SAPO, VACA) — e nunca com parada (B, P, T, D, C, G): "mmm-el" dá para ouvir separado, "b-ola" não. O `dinamicas.py` avisa na banca; Padaria e Trem foram reordenados |

### Números e medida

| mecânica | o gesto | a armadilha |
|---|---|---|
| **base-dez** | a criança **enche** a caixa e é ela quem manda trocar 10 por 1 | o conceito mora na TROCA, e a troca tem que ser VISÍVEL. ⭐ (set/2026, regra 5 da pesquisa — MLC/PhET) abre com **EXPLORAÇÃO LIVRE**: a mesma caixa sem juízo e o botão **Começar** (`var LIVRE=1; /*TECNICA*/`; `dadosExtra:{LIVRE:0}` desliga). O `_qa/dinamicas.py` avisa manipulável sem `LIVRE` |
| **balanca** | a criança **põe e tira peso** até a viga parar no meio — o "=" deixa de ser "aqui vem a resposta" | a viga GIRA com transição (balança que troca de figura não ensina); desequilíbrio **não é erro** (o andaime só entra quando ela se AFASTA); estado escrito, nunca só na cor. ⭐ (set/2026) abre com **EXPLORAÇÃO LIVRE** sem andaime e sem fechar rodada (`LIVRE`) |
| **reta-numerica** | **cravar** onde fica o número — um dos melhores preditores do desempenho matemático | a faixa inteira é o alvo (nunca o pixel), tolerância por rodada, mostra a DISTÂNCIA (não "errou"), andaime desvanece; três portas + teclado. ⭐ (set/2026) abre com **EXPLORAÇÃO LIVRE**: todos os traços à vista e o número do lugar aparece ao tocar (`LIVRE`) |
...
| **relogio** | a criança **move os ponteiros** (pedido do professor, com todas as letras) — e tem **2 MODOS**: `"ponteiros"` (padrão) e `"escrever"`, em que ela lê o relógio e digita a hora | os dois ponteiros não podem começar sobrepostos: ela não descobre que há dois para mexer. No modo `escrever`, a leitura em números embaixo do relógio tem que ficar DESLIGADA — ela diria a resposta antes da pergunta. ⭐ (set/2026, regra 5) abre com **EXPLORAÇÃO LIVRE**: relógio para mexer, hora aparecendo embaixo, botão **Começar** (`LIVRE`) |
...
| **ditado** (regra escrita em set/2026) | **a voz dita, a criança escreve**: a palavra chega só pelo OUVIDO e sai pela mão — a única fase em que ortografia e escuta trabalham juntas | a criança **não vê a palavra escrita** (senão é "monte a palavra"); botão **Ouvir de novo** grande e sem custo; frase **sem acento** (o teclado dela não tem a tecla); **duas portas** (teclado da tela + `document.onkeydown`); a tecla usada **sai do alcance mas guarda o lugar**; o `setTimeout` da rodada guarda a tela (`telaAtual.parentNode`), senão o teclado fica preso na fase anterior. ⭐ (set/2026) **andaime**: 1º dita de novo · 2º **VER A PALAVRA** 2,5 s e ela se esconde + a letra da vez pisca (EdiLIM "ver texto"; mapeamento ortográfico, Ehri) · 3º revela e segue. Portão: `_qa/dinamicas.py` (regra "ditado": duas portas, `mostraPalavra`, acento) |
