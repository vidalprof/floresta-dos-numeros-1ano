...
3. **Escolher os gestos** pelo ENCAIXE (§2), não por lista. → abrir o
   `_padrao/CARDAPIO.md` (as 81 peças por tipo de conteúdo; as 44 nunca usadas
   marcadas ⭐) e mirar ≥1 peça ⭐ que encaixe — chega de "sempre as mesmas".
...
## 🧬 CLONAR O JARDIM À MÃO — a lista curta (paga em 2026-08-12)

Quando o Marcos pedir uma atividade "do jeito antigo" (HTML único escrito à mão,
clonando o motor do Jardim do Broto), esta é a lista do que SEMPRE quebra. Ela é
menor que o `_padrao/CLONAR-MOTOR.md` porque aqui não há peças: é o motor puro.

1. **O CSS que vem junto é o do CONTEÚDO ANTIGO.** Toda classe que a fase nova
   inventar (`.palavraGrande`, `.opt.letra`, `.placa`, `.chapa`) nasce **sem
   regra**. O `node --check` passa, a tela abre, e a fase aparece sem forma
   nenhuma. Classe nova = regra no `<style>` **no mesmo commit** (`_qa/classes.py`).
2. **Não invente função do motor.** No Jardim, quem instala o alto-falante das
   respostas é o **MutationObserver** sobre `ZAPSEL` — não existe `poeVoz()`.
   Chamar uma função que não existe estoura a fase no clique (`_qa/funcoes.py`).
   Resposta nova para falar = pôr a classe dela no `ZAPSEL`.
3. **`data-qa` é a resposta CERTA DE AGORA.** Num par "sim/não" a resposta muda a
   cada rodada; marca fixa faz o auditor-jogador bater sempre no mesmo lado e dar
   "PRESO" numa fase que a criança fecha. E quem já foi achado **perde** o
   `data-qa` (senão o jogador fica clicando no mesmo).
4. **Entidade HTML não entra em texto que se pinta letra a letra.** `"CA_PE&#195;O"`
   percorrido com `charAt` vira `C A ? P E & # 1 9 5 ; O` na tela. Caractere de
   verdade (`Ã`), sempre.
5. **Enunciado é para ser FALADO.** Nada de `CA_PO` no balão nem de `<br>` dentro
   do texto que vira chave de voz (o `textContent` cola as duas linhas e a chave
   não bate com nada).
6. **Conferir a EXTENSÃO real do fundo** (`.png` no disco × `.jpg` no código).
7. **Trocar sempre:** `IMGS`, `VOZOK`, `DOM`/`CONC`/`ROTCRI`/`TREINO`, a chave do
   `localStorage`, o `PREFIXO` do `sw.js`, o `manifest.json`, o nome do mascote —
   e as falas da capa/"quem vai jogar" (o "Quem vai cuidar do jardim hoje?" passa
   despercebido porque não dá erro nenhum).
8. **O `falas.json` nasce junto** e o texto dele é o texto DO BALÃO, palavra por
   palavra — é o que o `_qa/vozigual.js` compara.
