...
(async () => {
  ...
  for (const vp of TAMANHOS) {
    ...
    for (let i = 0; i < quantas; i++) {
      ...
      try { await p.waitForFunction(() => !document.querySelector('.mt-entra,.casca-transicao'), {timeout:2000}); } catch (e) {}
      medidas++;
      ...
      const r = await p.evaluate(() => {
        ...
        document.querySelectorAll('#app *').forEach(e => {
          ...
          /* ⚠️ carta de MEMORIA e foto emoldurada sao brancas DE PROPOSITO
             (ago/2026, RIGHT NOW): a face da carta (`mface/mfrente/mcarta`) e um
             cartao, e a `.mfoto`/`.qfig` e a moldura branca da foto que o Marcos
             pediu ("dentro de moldura, foto no papel"). Nao e descuido — e design. */
          const ehPapel = /folha|quadro|papel|prancheta|canvas|desenh|mface|mfrente|mcarta|mfoto|moldura|foto|postit/i.test(
        ...
        document.querySelectorAll('#app *').forEach(pai => {
          ...
          /* ⚠️ EXCECAO QUE NAO E DESCUIDO, e ela e irma do "papel branco" logo
             acima: ha mecanica em que a FORMA DO CARTAO **e** o conteudo. No
             "monte a frase" cada vaga tem o feitio da classe de palavra que ela
             pede — pastilha para o artigo, tijolo para o nome, seta para a
             acao — e a crianca acerta OLHANDO o encaixe. Cobrar cantos iguais
             ali seria o portao mandando apagar a mecanica.
             O sinal e explicito, nao adivinhado: a peca marca essas familias
             com a classe `formas-de-proposito` no pai. */
          const deProposito = / formas-de-proposito /.test(' ' + String(pai.className) + ' ');
          if (cls.size === 1 && raios.size > 1 && !deProposito) {
