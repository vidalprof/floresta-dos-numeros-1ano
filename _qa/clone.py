...
# ---- 13) ⭐ ENFEITE CLONADO — o resto de clone que e CODIGO, nao arquivo
#   ⚠️ LICAO PAGA (ago/2026), e foi o Marcos jogando: *"tambem tem as borboletas
#   do Broto"*. O motor da Oficina veio do Jardim e trouxe junto o pano de fundo
#   vivo — TRES BORBOLETAS voando, desenhadas no canvas. Numa oficina de
#   letreiros, no meio de uma rua de Blumenau.
#
#   Nenhum item anterior pegava, e o motivo importa: este resto nao e imagem
#   (item 1), nem voz (2, 3), nem prefixo (8), nem nome (10, 12). E CODIGO QUE
#   DESENHA — e codigo que desenha nao tem nome de arquivo para conferir.
#
#   A regra que da para medir: o enfeite de fundo desenha uma COISA DO MUNDO
#   (borboleta, semente, planeta, papagaio...). Se essa palavra existe igual no
#   codigo de OUTRA atividade e a atividade daqui NUNCA fala dela — nem na tela,
#   nem nas falas — entao ela veio de carona. O que e do mundo desta atividade
#   aparece no que a crianca le e ouve; o que veio de carona, nunca.
# so COISAS DE UM MUNDO — bicho, planta, corpo celeste, veiculo. Efeito
# generico (faisca, gota, poeira, floco) e do MOTOR e pertence a todo mundo:
# na primeira versao esta lista os incluia e o portao acusou o inocente, que e
# o jeito mais rapido de ensinar a ignorar portao.
COISAS = ("borboleta", "semente", "broto", "planeta", "orbita", "papagaio",
          "navio", "peixe", "abelha", "vagalume", "flor", "raiz", "fruto",
          "foguete", "dinossauro", "caravela")
# ⚠️ O COMENTARIO CONTA — e foi ele que teria denunciado as borboletas.
#    O nome da variavel era `borbs` (que nao casa com palavra nenhuma), mas o
#    cabecalho do bloco dizia, com todas as letras, "BRILHO VIVO (polen +
#    borboletas)". Quem clona o motor clona o comentario junto: ele e a
#    etiqueta de origem.
#    ⚠️ Ficam de fora os BLOCOS de licao inteiros (os que tem ⚠️ em qualquer
#    linha): eles falam justamente do defeito ja consertado, e nomear o defeito
#    e o trabalho deles. Filtrar linha a linha nao servia — a licao ocupa
#    quinze linhas e so a primeira leva o simbolo.
def _sem_licoes(t):
    saida, i = [], 0
    ...
    saida.append(t[i:])
    fora = "".join(saida)
    return "\n".join(l for l in fora.split("\n")
                      if "\u26a0" not in l and "LICAO" not in l.upper())
_semcom = _sem_licoes(html)
_meu = _semcom.lower()
_camf = os.path.join(pasta, "falas.json")
_carona = []
for _coisa in COISAS:
    if _coisa not in _meu:
        continue
    # ela e do MUNDO daqui? entao aparece no texto que a crianca le/ouve
    # ⚠️ o que a crianca VE nao se procura no arquivo cru: entre duas aspas
    #    quaisquer cabe um comentario inteiro, e foi assim que a palavra
    #    "borboletas" — que so existia num COMENTARIO — passou por "texto da
    #    tela" e absolveu o enfeite clonado. Tirar o comentario primeiro.
    _sotexto = re.sub(r"/\*.*?\*/", " ", html, flags=re.S)
    _sotexto = re.sub(r"(?m)^\s*//.*$", " ", _sotexto)
    _texto_da_crianca = re.findall(r'"([^"\n]{4,})"', _sotexto)
    _visivel = " ".join(_texto_da_crianca).lower()
    if os.path.exists(_camf):
        _visivel += " " + open(_camf, encoding="utf-8", errors="replace").read().lower()
    if _coisa in _visivel:
        continue
    # aparece igual no codigo de outra atividade? entao veio de la
    _outras = []
    _raiz5 = os.path.dirname(os.path.abspath(pasta)) or "."
    for _o in sorted(os.listdir(_raiz5)):
        _c = os.path.join(_raiz5, _o, "index.html")
        if not e_vizinha(_o) or not os.path.exists(_c):
            continue
        if os.path.abspath(os.path.dirname(_c)) == os.path.abspath(pasta):
            continue
        if _coisa in open(_c, encoding="utf-8", errors="replace").read().lower():
            _outras.append(_o)
    if _outras:
        _carona.append((_coisa, _outras[:2]))
if _carona:
    problemas.append("o enfeite desenha coisa que esta atividade nunca menciona e que "
                     "existe igual noutra: %s — enfeite clonado (o resto de clone que "
                     "e CODIGO, nao arquivo)"
                     % ", ".join("%s (tambem em %s)" % (c, ", ".join(o)) for c, o in _carona[:3]))
else:
    print("   enfeite: o fundo vivo so desenha coisa do mundo desta atividade")

# ---------------------------------------------------------------------------
# 14) ENREDO CLONADO — a VOZ conta uma historia e o TEXTO conta outra
#
# ⚠️ LICAO PAGA (ago/2026, no Tangram da Vovo Marta). A tela de abertura dizia,
# ESCRITO: *"Oi! Eu sou a Marta, pintora de letreiros. O meu ajudante trocou uma
# letra em quase todas as placas da rua..."* — a historia da Oficina da Lina,
# inteirinha, dentro de um jogo de tangram. E a VOZ, gravada do `falas.json`
# desta pasta, dizia a historia certa: *"A caixa de tangram da vovo Marta caiu
# no chao..."*.
#
# Os doze itens anteriores olhavam ARQUIVO (imagem, mp3, prefixo, nome, enfeite).
# Nenhum olhava o ENREDO. E o enredo e o resto de clone mais caro de todos: nao
# quebra nada, o `node --check` passa, a arte esta certa — e a crianca que le
# recebe uma historia, a que nao le recebe outra.
#
# COMO SE MEDE, sem adivinhar: quando uma funcao `telaX` escreve um `balao` e
# chama `falar("id")`, o texto do balao TEM que ser o texto de `id` no
# `falas.json` desta pasta. Sao a mesma frase dita duas vezes; se divergirem
# alem de um retoque, uma das duas veio de outro lugar.
# ---------------------------------------------------------------------------
def _achata_frase(t):
    t = re.sub(r"<[^>]+>", " ", t or "")
...
_falasj = os.path.join(pasta, "falas.json")
_divergem = []
if os.path.exists(_falasj):
    try:
        _fj = json.loads(io.open(_falasj, encoding="utf-8").read())
    except ValueError:
        _fj = []
    _porid = dict((f.get("id"), f.get("texto") or "") for f in _fj if f.get("id"))
    for _m in re.finditer(r"^function\s+(tela\w+)\s*\(", html, re.M):
        _ini = html.find("{", _m.end())
        _p, _k = 0, _ini
        while _k < len(html):
            if html[_k] == "{":
                _p += 1
        ...
        _tv = _porid.get(_ids[0])
        if not _tv:
            continue
        _a, _b = set(_achata_frase(_bal[0])), set(_achata_frase(_tv))
        if not _a or not _b:
            continue
        _com = len(_a & _b) / float(min(len(_a), len(_b)))
        if _com < 0.45:       # menos de 45% das palavras em comum = outra historia
            _divergem.append((_m.group(1), _ids[0],
                              " ".join(_achata_frase(_bal[0]))[:52],
                              " ".join(_achata_frase(_tv))[:52]))
if _divergem:
    problemas.append("a VOZ e o TEXTO contam historias diferentes (enredo clonado): "
                     + "; ".join('%s escreve "%s..." e a voz %s diz "%s..."'
                                 % (t, e, i, v) for t, i, e, v in _divergem[:3]))
elif os.path.exists(_falasj):
    print("   enredo: o texto escrito conta a mesma historia que a voz gravada")
