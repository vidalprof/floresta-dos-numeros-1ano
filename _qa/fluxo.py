...
def sem_as_pecas(html):
    u"""⚠️⚠️ LICAO PAGA (ago/2026) — O NOME QUE EXISTE DUAS VEZES, EM ESCOPOS
    DIFERENTES, e o portao lia os dois como se fossem um.

    A peca `quem-sou-eu` chama a tela dela de `telaQuem` — e o MOTOR chama de
    `telaQuem` a tela "quem vai jogar". Dentro da atividade nao ha conflito
    nenhum: cada peca vive dentro do seu `MEC[...] = function(){...}`, entao a
    `telaQuem` da peca e LOCAL e nao encosta na do motor.

    Este portao, porem, varria o arquivo inteiro com um regex chato e guardava
    a ULTIMA definicao de cada nome. Ficava com a `telaQuem` da PECA — que
    naturalmente nao chama `telaMestre` nem `telaAbertura` — e entao as duas
    telas do motor apareciam como ORFAS. Duas acusacoes falsas, nas DUAS
    atividades, e foram as ULTIMAS a segurar a banca.

    O conserto e respeitar o escopo: para desenhar o fluxo do MOTOR, o corpo
    das pecas sai do texto. O que acontece dentro delas ja e medido por
    `confere_esqueleto` e pelo auditor-jogador, que joga de verdade."""
    fora, i = [], 0
...
def telas_e_saidas(html):
    ...
    # o fluxo desenhado aqui e o do MOTOR; o de dentro das pecas tem outra
    # regua (ver `sem_as_pecas` e `confere_esqueleto`).
    if re.search(r'MEC\[\s*"[\w-]+"\s*\]\s*=\s*function', html):
        html = sem_as_pecas(html)
