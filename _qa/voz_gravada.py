...
============================================================
 ...
 ⭐ POR QUE ESTE PORTÃO EXISTE — o defeito de 23/set/2026, que o carimbo NÃO
    ...
    e o site foi ao ar com o texto novo e **catorze falas sem mp3** — enquanto o
    carimbo `_status/entrega-<repo>.json` dizia `"noar":1` com o sha certo,
    porque o sha do `index.html` estava certo mesmo. **O carimbo mede a PÁGINA,
    não a VOZ**, e era esse o buraco.
 ...
 ⚠️ **A CAUSA eu NÃO confirmei, e não vou inventá-la.** O que está medido: o
    ...
    das duas diria. **O que eu consertei não foi a causa: foi a MEDIDA**, que
    serve para qualquer causa.
...
def main():
    ...
    print(u"   conserto: acionar o `entregar.yml` para esta pasta — ele grava o "
          u"que falta (foi assim que as 14 do _sil2 chegaram).")
    print(u"   ⚠️ e evitar entregar a mesma pasta duas vezes em poucos minutos: "
          u"foi o que eu fiz no dia em que isto apareceu.")
    return 1
