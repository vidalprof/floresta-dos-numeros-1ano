...
# codigo(<nome>, <codigo esperado>, <comando...>) — quando "reprovou" nao basta.
# O `errador` tem TRES saidas e a diferenca entre elas e o coracao da casa:
#   0 = mediu e passou · 1 = REPROVOU · 2 = nao consegui medir (nao reprova).
# Confundir 1 com 2 e o erro que esta serie toda pagou.
codigo(){
  local nome="$1" esperado="$2"; shift 2
  "$@" > "$T/saida.txt" 2>&1
  local cod=$?
  if [ "$cod" = "$esperado" ]; then echo "   ok      $nome (codigo $cod, como devia)"
  else echo "   ERRADO  $nome — esperava codigo $esperado e veio $cod:"
       sed -n '2,5p' "$T/saida.txt" | sed 's/^/           /'; falhou=1; fi
}
...
# ---------- 23) ERRADOR: reprovar x "nao medi" ----------
# O portao do ANDAIME tem tres saidas, e a casa inteira depende de ele nao
# confundir duas delas: REPROVAR uma peca boa manda consertar o que esta certo;
# dizer "passou" sem medir e aprovacao vazia. Aqui as duas pontas sao plantadas.
mkdir -p "$T/_and"
peca_andaime(){ # peca_andaime(<dica do 2o erro>, <dica do 3o erro>)
cat <<H
<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><style>
#app{padding:12px}.opt{display:block;min-height:48px;margin:8px;background:#eee}
.dica{background:#ffd}.medal{font-size:30px}
</style></head><body><div id="app"></div><script>
var erros=0;
function el(t,c,h){var e=document.createElement(t);if(c)e.className=c;if(h)e.innerHTML=h;return e;}
function mostraDica(t){var d=document.getElementById("dicaP");
  if(!d){d=el("div","dica");d.id="dicaP";document.getElementById("app").appendChild(d);} d.innerHTML=t;}
function tela(){
  var app=document.getElementById("app"); app.innerHTML="";
  var i,ns=["um","dois","tres","certa"];
  for(i=0;i<4;i++){
    var o=el("div","opt",ns[i]);
    o.setAttribute("data-qa", i===3?"1":"0");
    o.onclick=(function(k){ return function(){
      if(k===3){ app.innerHTML='<div class="medal">estrela</div>'; return; }
      erros++;
      if(erros===1) mostraDica("Olhe com calma.");
      else if(erros===2) mostraDica("$1");
      else mostraDica("$2");
    };})(i);
    app.appendChild(o);
  }
...
# andaime que NAO cresce: a mesma frase nos tres erros -> tem que REPROVAR (1)
peca_andaime "Olhe com calma." "Olhe com calma." > "$T/_and/parado.html"
# andaime que cresce: tres frases diferentes -> tem que PASSAR (0)
peca_andaime "A certa esta piscando." "Era esta! Eu marco para voce." > "$T/_and/cresce.html"
codigo "errador  · andaime que nao cresce" 1 node _qa/errador.js "$T/_and/parado.html"
codigo "errador  · andaime que cresce"     0 node _qa/errador.js "$T/_and/cresce.html"

# ---------- 22) FALA O ESCRITO: as DUAS formas de gravar a mesma frase ----------
# Defeito original (Padaria, ago/2026): a tela mostra um balao e NAO ha gravacao
# nenhuma daquele texto -> a fala automatica some e o pequeno fica sem ouvir.
# Falso-positivo que veio depois (Jardim do Broto): a atividade escrita a mao
# grava a MESMA frase com NOME PROPRIO (`jd_abertura`) em vez da conta do texto,
# e o portao chamou de "muda" uma tela que fala perfeitamente. As duas coisas
# tem que valer ao mesmo tempo: pegar a muda de verdade, deixar passar a falada.
mkdir -p "$T/_fe/audio"
fe_ativ(){ # $1 = texto gravado no falas.json
cat > "$T/_fe/falas.json" <<J
[{"id":"jd_intro","texto":"$1"}]
J
cat > "$T/_fe/index.html" <<'H'
<!DOCTYPE html><html><head><meta charset="utf-8"></head><body><div id="app"></div><script>
function el(c,k,t){var d=document.createElement(c);d.className=k||"";if(t)d.innerHTML=t;return d;}
function chaveVoz(t){var h=5381,i;for(i=0;i<t.length;i++)h=((h*33)^t.charCodeAt(i))>>>0;return h.toString(36);}
function telaUm(){var a=document.getElementById("app");a.innerHTML="";
  var t=el("div","tela");t.appendChild(el("div","balao","Ache as quatro palavras escondidas."));
  a.appendChild(t);}
</script></body></html>
H
}
fe_ativ "Ache as quatro palavras escondidas."
pega "falaescrito · balao gravado por NOME proprio" DEIXA node _qa/fala_o_escrito.js "$T/_fe"
fe_ativ "Uma frase completamente diferente do balao."
pega "falaescrito · balao sem gravacao nenhuma"     PEGA  node _qa/fala_o_escrito.js "$T/_fe"

# ---------- 23) AJUDA FORA DO #dicaP: a medida ampla ve o que a estreita nao ve ----------
# A `quem-sou-eu` socorre a crianca com uma PISTA NOVA num quadro proprio, nao
# com um `#dicaP` — e o portao, que so lia o `#dicaP`, chamava isso de "andaime
# parado". A medida ampla (todo texto que APARECE na tela depois do erro)
# existe para isso. Aqui ela e provada nos dois sentidos: ve a ajuda que mora
# fora, e nao inventa ajuda quando nada aparece.
mkdir -p "$T/_amp"
peca_ampla(){ # $1 = "sim" (ajuda nova a cada erro) | "nao" (nada aparece)
cat > "$T/_amp/p.html" <<H
<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>
<div id="app"></div><div id="banner" class="banner"><div id="bmsg"></div>
<button id="bcta" class="btn">Continuar</button></div>
<script>
var erros=0, MODO="$1";
function el(t,c,x){var d=document.createElement(t);d.className=c||"";if(x)d.innerHTML=x;return d;}
/* cada erro tem que trazer uma pista INEDITA: se a peca de mentira repetisse
   a ultima, a medida ampla acertaria em dizer "nao apareceu nada novo" — e eu
   quase acusei o portao por um defeito do meu proprio caso de teste. */
var LUGARES=["do vaso","da janela","do regador","da porta","do teto","do chao",
             "da mesa","da estante","do tapete","da cortina"];
function tela(){
  var app=document.getElementById("app"); app.innerHTML="";
  var q=el("div","pistas","O que se sabe ate agora:"); q.id="quadro"; app.appendChild(q);
  var i,ns=["um","dois","tres","certa"];
  for(i=0;i<4;i++){
    var o=el("div","opt",ns[i]);
    o.setAttribute("data-qa", i===3?"1":"0");
    o.onclick=(function(k){ return function(){
      if(k===3){ app.innerHTML='<div class="medal">estrela</div>'; return; }
      if(MODO==="sim"){
        var q=LUGARES[erros%LUGARES.length]+" "+erros;
        var p=el("div","pist","A pista "+q+" conta uma coisa que ninguem tinha visto ainda.");
        document.getElementById("quadro").appendChild(p);
      }
      erros++;
    };})(i);
    app.appendChild(o);
...
# a ajuda mora FORA do #dicaP e cresce -> a medida ampla tem que VER as tres
peca_ampla sim
node _qa/errador.js "$T/_amp/p.html" > "$T/amp1.txt" 2>&1
if grep -q "ajuda NOVA na tela (qualquer lugar): 3 de 3" "$T/amp1.txt"; then
  echo "   ok      ampla    · viu a ajuda que mora fora do #dicaP"
else
  echo "   CEGA    ampla    · nao viu a pista nova:"; grep "ajuda NOVA" "$T/amp1.txt" | sed 's/^/           /'; falhou=1
fi
# nada aparece -> ela NAO pode inventar ajuda
peca_ampla nao
node _qa/errador.js "$T/_amp/p.html" > "$T/amp2.txt" 2>&1
if grep -q "ajuda NOVA na tela (qualquer lugar): 0 de 3" "$T/amp2.txt"; then
  echo "   ok      ampla    · nao inventou ajuda onde nao houve"
else
  echo "   INVENTOU ampla   · viu ajuda numa tela que nao mudou:"; grep "ajuda NOVA" "$T/amp2.txt" | sed 's/^/           /'; falhou=1
fi

# ---------- 24) BECO SEM SAIDA: a tela de bancada da peca virando fim de linha ----------
# O defeito mais grave que a fabrica ja teve: a crianca termina a fase 3 de 32,
# cai na tela "PECA FECHADA" da peca (a de testar a peca sozinha) e o unico
# botao e "Jogar de novo". Recomeca a MESMA fase, para sempre.
beco_ativ(){ # $1 = "religada" | "solta"
cat > "$T/beco.html" <<H
...
MEC["intruso"] = function(f, cen, fim){
  ...
  (function(){
    function fimDaPeca(){
      ...
      c.innerHTML='<div class="selo">PE&#199;A FECHADA</div><div class="medal"></div>'
                 +'<button class="btn">Jogar de novo</button>'
                 +'<div class="hint">Esta &#233; a pe&#231;a O INTRUSO.</div>';
    }
    function pecaIntruso(){ fimDaPeca(); }
    ${1}
    pecaIntruso();
  })();
};
...
# a ponte RELIGOU o fim da peca: a fase leva para a seguinte -> tem que PASSAR
beco_ativ 'try{ fimDaPeca = _seguir; }catch(_e){}'
pega "beco     · ponte religou o fim da peca" DEIXA python3 _qa/beco.py "$T/beco.html"
# a ponte NAO religou: a tela de bancada vira fim de linha -> tem que REPROVAR
beco_ativ ''
pega "beco     · tela de bancada virou fim de linha" PEGA python3 _qa/beco.py "$T/beco.html"

# ---------- 25) A FIGURA COMBINA COM A PALAVRA? ----------
# O Marcos viu na tela: "e por exemplo um abacate, esta escrito ovo e o som
# esta ovo". No conteudo estava "nome":"OVO" com "img":"pd_mamao".
mkdir -p "$T/_fg"
fig_ativ(){ cat > "$T/_fg/conteudo.json" <<J
{"prefixo":"pd","fases":[{"id":"f01","mec":"ouvir-achar",
 "dadosExtra":{"CATALOGO":{"pd_pao":{"nome":"PAO","img":"pd_pao"},
                           "pd_ovo":{"nome":"OVO","img":"$1"}}}}]}
J
}
fig_ativ pd_ovo
pega "figura   · o desenho tem o nome da palavra" DEIXA python3 _qa/figura_certa.py "$T/_fg"
fig_ativ pd_mamao
pega "figura   · OVO apontando para o mamao"      PEGA  python3 _qa/figura_certa.py "$T/_fg"

# ---------- 26) VOZ-ROBO: a palavra que ficou muda ----------
# O Marcos: "onde tenho que ouvir a palavra nao funciona, o enunciado funciona".
# A ponte desliga a voz do navegador (defeito anterior: duas vozes juntas) e a
# peca usava essa voz para dizer A PALAVRA. Conserto de um virou defeito do outro.
robo_ativ(){ cat > "$T/robo.html" <<H
<!DOCTYPE html><html><head><meta charset="utf-8"></head><body><script>
var MEC={};
...
robo_ativ 'var diz = function(t){ var k=temVoz(t); if(k){ tocaVoz(k); return; } };'
pega "vozrobo  · a ponte religou a voz da peca" DEIXA python3 _qa/vozrobo.py "$T/robo.html"
robo_ativ ''
