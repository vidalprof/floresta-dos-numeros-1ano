...
## isto produz um mundo novo sem quebrar o que já funciona.
## Fontes-mãe (NÃO duplicadas aqui — leia lá o detalhe): IDEIA-MUNDO-VIVO.md
## (visão) + MUNDO-VIVO-MOTOR.md (manual técnico do motor, §1-§8).
## Referência viva do motor: `_pub_confeitaria/mundo/index.html`.
...
- Pé patinando ao acelerar → trocar quadro por DISTÂNCIA (~34px), nunca por
  tempo fixo.
- Clique engolido pelo arrasto → guarda de 280ms (`MM.tDrop`).
- Fala velha invadindo a cena nova → `_encadear=false; pararFala()` na
  troca de desafio.
...
- Mascote errado extraído do HTML de origem → ancorar busca em
  `MASCOTE_POSES` (ou objeto próprio do jogo), SEMPRE conferir a imagem
  com o usuário antes de fechar.
...
- Interior "piscando" no boot → vestir a cena SÓ dentro de `abrirParada`,
